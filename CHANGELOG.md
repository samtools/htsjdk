...
- `StructuralVariantType` gains `MIXED` and a non-throwing `parse` method that extracts the major type from `DEL:ME:ALU` or `<DEL:ME:ALU>`, with a `parse(byte[])` overload that allocates nothing. `StructuralVariantAllele` is a new immutable value type that carries the major type and the ordered subtype list, and says through `isSymbolic()` and `isBreakend()` which kind of allele it describes. A breakend's `getBreakend()` returns the new `Breakend`, parsed from any of the specification's breakend forms (the four paired forms, single breakends, assembly contigs such as `C[<ctg1>:1[`, telomeres such as `.[13:123457[`): its bases and inserted bases, whether the mate is joined after or before them (`isJoinedAfter()`, `isJoinedBefore()`), and the mate's contig, 1-based position and strand (`isMatePositiveStrand()`, `isMateNegativeStrand()`). An allele whose breakend notation is malformed is not a structural variant; `Allele.asStructuralVariant()` returns one for symbolic SV and breakend alleles. `VariantContext.getStructuralVariantType()` consults both `SVTYPE` and the alleles: no SV alleles returns null, one major type returns it, more than one or an SVTYPE-vs-allele disagreement returns `MIXED`.
...
- **Unsigned `B` arrays keep their type through CRAM** (issue #499).  The writer stored `B:C`, `B:S` and `B:I` arrays as signed, and the reader returned every array as signed, so `B:C,200` came back as `B:c,-56`.
...
- **`getAttributeAsInt`, `getAttributeAsIntList` and `getAttributeAsDoubleList` return the default for a missing value (`.`) read from VCF** (issue #1228); they compared it by identity, so `X=.` or `X=1,.,3` threw `NumberFormatException`.
