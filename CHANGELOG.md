...
- An index written along with a BAM ends the file's last chunk where samtools does: the BGZF stream is flushed before the file pointer is taken, giving start-of-next-block offset 0, matching `hts_idx_finish`'s `final_offset`.
...
- **New: on-the-fly indexing of bgzipped SAM.**  `SAMFileWriterFactory.setCreateIndex(true)` writing `x.sam.gz` from coordinate-sorted records writes an index beside it, as it does for a BAM.  The default index type is CSI (written as `x.sam.gz.csi`, as samtools writes on the fly); `setSamIndexType(BamIndexType)` chooses BAI (`x.sam.gz.bai`) or AUTO.  A CSI for SAM text always carries a tabix header naming every sequence of the header, so samtools and tabix both read it.  `setCsiMinShift` applies to both SAM and BAM CSI indexes.
