...
- **A tab or line break in a header value, a header comment, a read name or a Z or A tag no longer corrupts the SAM text written** (issue #1108).  SAM text has no escapes, so such a value split its field or its line: a read group whose description was `x<TAB>PI:123` was read back with a `PI` tag.  Writing such a header (to SAM, BAM or CRAM, whose headers are SAM text) or such a record (to SAM) now throws an `IllegalArgumentException` that shows the value.  Tabs are still allowed in `@CO` comments.
