...
- **BCF output is BCF 2.2 with BGZF compression by default.**  A caller that needs raw BCF, or a Tribble `.idx` index over one, selects `setBCFVersion(BCFVersion.BCF_2_1)` on the `VariantContextWriterBuilder`.  A builder that still has `INDEX_ON_THE_FLY` (a default option) and a BCF 2.2 output now throws at `build()`, before anything is opened: drop the option with `unsetOption(Options.INDEX_ON_THE_FLY)` or `clearOptions()`, or choose BCF 2.1.  CSI indexing of BGZF BCF is not yet available.
