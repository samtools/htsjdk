...
public class CompressionHeaderFactory implements CramCompression {
     ...
     */
    public CompressionHeader buildCompressionHeader(final List<CramCompressionRecord> records, SubstitutionMatrix substitutionMatrix,
    ...
    static int getTagValueByteSize(final byte type, final Object value) {
        switch (type) {
            ...
            case 'B':
                ...
                throw new IllegalArgumentException("Unknown tag array class: " + value.getClass());
            default:
    ...
    private static ByteSizeStats getMinByteSizeOfTagValues(final List<CramCompressionRecord> records, final int tagID) {
        ...
        for (final CramCompressionRecord record : records) {
            ...
            for (final ReadTag tag : record.tags) {
                ...
                int size = getTagValueByteSize(type, tag.getValue());
                if (stats.min < size)
                    stats.min = size;
                if (stats.max > size)
                    stats.max = size;
