...
/**
 * SAMFileWriter that can be wrapped around an underlying SAMFileWriter to provide asynchronous
 * output. Records added are placed into a queue, the queue is then drained into the underlying
 * SAMFileWriter by a thread owned by the instance.
 *
 * <p>Exceptions experienced by the writer thread will be emitted back to the caller in subsequent
 * calls to either addAlignment() or close().
 *
 * @author Tim Fennell
 */
class AsyncSAMFileWriter extends AbstractAsyncWriter<SAMRecord> implements SAMFileWriter {
  ...
  /**
   * Creates an AsyncSAMFileWriter wrapping the provided SAMFileWriter and using the specified queue
   * size for buffer SAMRecords.
   */
  ...
  /**
   * Adds an alignment to the queue to be written. Will re-throw any exception that was received
   * when writing prior record(s) to the underlying SAMFileWriter.
   */
  @Override
