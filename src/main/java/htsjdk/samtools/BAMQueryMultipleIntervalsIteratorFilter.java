...
public class BAMQueryMultipleIntervalsIteratorFilter implements BAMIteratorFilter {
  ...
  public BAMQueryMultipleIntervalsIteratorFilter(
  ...
  public FilteringIteratorState compareToFilter(final SAMRecord record) {
    while (intervalIndex < intervals.length) {
      final IntervalComparison comparison =
          compareIntervalToRecord(intervals[intervalIndex], record);
      switch (comparison) {
  ...
  public static IntervalComparison compareIntervalToRecord(
      final QueryInterval interval, final SAMRecord record) {
    ...
    else if (CoordMath.encloses(
        interval.start, intervalEnd, record.getAlignmentStart(), alignmentEnd)) {
