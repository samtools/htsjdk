...
public class BamFileIoUtils {
  ...
  public static void reheaderBamFile(
      final SAMFileHeader samFileHeader, final File inputFile, final File outputFile) {
   ...
   * @param outputFile The new BAM file, constructed with the new header and the content from
   *     inputFile
   ...
   * @param skipTerminator If true, the terminator block of the input file will not be written to
   *     the output stream
   */
  public static void blockCopyBamFile(
    ...
    try {
      ...
      // a) It's good to check that the end of the file is valid and b) we need to know if there's a
      // terminator block and not copy it if skipTerminator is true
      ...
      if (term == BlockCompressedInputStream.FileTermination.DEFECTIVE)
        throw new SAMException(
            inputFile.getAbsolutePath()
                + " does not have a valid GZIP block at the end of the file.");
      ...
      final long skipLast =
          ((term == BlockCompressedInputStream.FileTermination.HAS_TERMINATOR_BLOCK)
                  && skipTerminator)
  ...
  /**
   * Assumes that all inputs and outputs are block compressed VCF files and copies them without
   * decompressing and parsing most of the gzip blocks. Will decompress and parse blocks up to the
   * one containing the end of the header in each file (often the first block) and re-compress any
   * data remaining in that block into a new block in the output file. Subsequent blocks (excluding
   * a terminator block if present) are copied directly from input to output.
   */
  public static void gatherWithBlockCopying(
    ...
    try {
      ...
      if (createIndex) {
        indexFile =
            new File(output.getParentFile(), IOUtil.basename(output) + BAMIndex.BAMIndexSuffix);
      ...
      // It is possible that the modified time on the index file is ever so slightly older than the
      // original BAM file
  ...
  private static OutputStream buildOutputStream(
      final File outputFile, final boolean createMd5, final boolean createIndex)
      throws IOException {
    OutputStream outputStream = new FileOutputStream(outputFile);
    if (createMd5) {
      outputStream =
          new Md5CalculatingOutputStream(
              outputStream, new File(outputFile.getAbsolutePath() + ".md5"));
    }
    if (createIndex) {
      outputStream =
          new StreamInflatingIndexingOutputStream(
              outputStream,
              new File(
                  outputFile.getParentFile(),
                  IOUtil.basename(outputFile) + BAMIndex.BAMIndexSuffix));
    }
    return outputStream;
  }
  ...
  private static void assertSortOrdersAreEqual(final SAMFileHeader newHeader, final File inputFile)
    ...
    if (newSortOrder != SAMFileHeader.SortOrder.unsorted
        && newSortOrder != origHeader.getSortOrder()) {
      throw new SAMException(
