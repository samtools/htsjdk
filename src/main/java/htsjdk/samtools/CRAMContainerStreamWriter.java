...
public class CRAMContainerStreamWriter {
  ...
  /**
   * Create a CRAMContainerStreamWriter for writing SAM records into a series of CRAM containers on
   * output stream, with an optional index.
   *
   ...
   * @param samFileHeader {@link SAMFileHeader} to be used. Sort order is determined by the
   *     sortOrder property of this arg.
   ...
   *
   * @param writeEOFContainer true if an EOF container should be written. Only use false if writing
   *     a CRAM file fragment which will later be aggregated into a complete CRAM file.
   */
  ...
  /**
   * Decide if the current container should be completed and flushed. The decision is based on a)
   * number of records and b) if the reference sequence id has changed.
   *
   * @param nextRecord the record to be added into the current or next container
   * @return true if the current container should be flushed and the following records should go
   *     into a new container; false otherwise.
   */
  protected boolean shouldFlushContainer(final SAMRecord nextRecord) {
    ...
    if (refSeqIndex != SAMRecord.NO_ALIGNMENT_REFERENCE_INDEX
        && nextRecord.getReferenceIndex() == SAMRecord.NO_ALIGNMENT_REFERENCE_INDEX) {
      return true;
    }
    ...
    /**
     * Protection against too small containers: flush at least X single refs, switch to multiref
     * otherwise.
     */
    ...
    for (final SAMRecord samRecord : samRecords) {
      if (samRecord.getAlignmentStart() != SAMRecord.NO_ALIGNMENT_START) {
        ...
        for (final CigarElement cigarElement : samRecord.getCigar().getCigarElements()) {
          ...
          refPos +=
              cigarElement.getOperator().consumesReferenceBases() ? cigarElement.getLength() : 0;
        }
      }
    }
  }
  ...
  protected void flushContainer()
      ...
      case Slice.MULTI_REFERENCE:
        if (preservation != null && preservation.areReferenceTracksRequired()) {
          throw new SAMException(
              "Cannot apply reference-based lossy compression on non-coordinate sorted reads.");
        }
        refs = new byte[0];
        break;
    ...
    final List<CramCompressionRecord> cramRecords =
        new ArrayList<CramCompressionRecord>(samRecords.size());

    final Sam2CramRecordFactory sam2CramRecordFactory =
        new Sam2CramRecordFactory(refs, samFileHeader, cramVersion);
    ...
    for (final SAMRecord samRecord : samRecords) {
      if (samRecord.getReferenceIndex() != SAMRecord.NO_ALIGNMENT_REFERENCE_INDEX
          && refSeqIndex != samRecord.getReferenceIndex()) {
        // this may load all ref sequences into memory:
        sam2CramRecordFactory.setRefBases(
            source.getReferenceBases(
                samFileHeader.getSequence(samRecord.getReferenceIndex()), true));
      }
...

    {
      if (samFileHeader.getSortOrder() == SAMFileHeader.SortOrder.coordinate) {
        // mating:
        final Map<String, CramCompressionRecord> primaryMateMap =
            new TreeMap<String, CramCompressionRecord>();
        final Map<String, CramCompressionRecord> secondaryMateMap =
            new TreeMap<String, CramCompressionRecord>();
        for (final CramCompressionRecord r : cramRecords) {
          if (!r.isMultiFragment()) {
            ...
            final Map<String, CramCompressionRecord> mateMap =
                r.isSecondaryAlignment() ? secondaryMateMap : primaryMateMap;
    ...
    {
      /**
       * The following passage is for paranoid mode only. When java is run with asserts on it will
       * throw an {@link AssertionError} if read bases or quality scores of a restored SAM record
       * mismatch the original. This is effectively a runtime round trip test.
       */
      ...
      if (assertsEnabled) {
        ...
        for (int i = 0; i < samRecords.size(); i++) {
          ...
          assert (restoredSamRecord
              .getReferenceName()
              .equals(samRecords.get(i).getReferenceName()));
          ...
          if (!restoredSamRecord.getReadString().equals(samRecords.get(i).getReadString())) {
            ...
            final String originalReadBasesUpperCaseIupacNoDot =
                new String(SequenceUtil.toBamReadBasesInPlace(originalReadBases));
            assert (restoredSamRecord.getReadString().equals(originalReadBasesUpperCaseIupacNoDot));
          }
          assert (restoredSamRecord
              .getBaseQualityString()
              .equals(samRecords.get(i).getBaseQualityString()));
        }
      }
    }
    ...
    if (indexer != null) {
      /**
       * Using silent validation here because the reads have been through validation already or they
       * have been generated somehow through the htsjdk.
       */
