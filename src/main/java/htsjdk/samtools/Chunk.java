...
/**
 * A [start,stop) file pointer pairing into the BAM file, stored as a BAM file index. A chunk is
 * represented as a single 64-bit value where the high-order 48 bits point to the location of the
 * start of a compressed BGZF block within a BGZF file and the low-order 16 bits point to a position
 * within the decompressed data in the BGZF block.
 *
 * <p>See the SAM/BAM spec for more details.
 */
public class Chunk implements Cloneable, Serializable, Comparable<Chunk> {
  private static final long serialVersionUID = 1L;

  /**
   * A pointer to the start of a region in a SAM/BAM file. The start is inclusive: start reading
   * from this point.
   */
  private long mChunkStart;

  /**
   * A pointer to the end of a region in a SAM/BAM file. The end is exclusive: this pointer points
   * to one byte past the end of the region of interest inside the file.
   */
  ...
  public boolean overlaps(final Chunk other) {
    ...
    final long leftMostBlockAddress =
        BlockCompressedFilePointerUtil.getBlockAddress(leftMost.getChunkEnd());
    final long rightMostBlockAddress =
        BlockCompressedFilePointerUtil.getBlockAddress(rightMost.getChunkStart());
    ...
    if (leftMostBlockAddress > rightMostBlockAddress) return true;
      ...
      final int leftMostOffset =
          BlockCompressedFilePointerUtil.getBlockOffset(leftMost.getChunkEnd());
      final int rightMostOffset =
          BlockCompressedFilePointerUtil.getBlockOffset(rightMost.getChunkStart());
   ...
   *
   * @param other Chunk to which this should be compared.
   * @return True if the two chunks are adjacent. Returns false if the chunks overlap or are
   *     discontinuous.
   */
  public boolean isAdjacentTo(final Chunk other) {
    // Simpler implementation would be to == the chunk end of one to the chunk start of the other.
    // Chose this implementation to ensure that all chunk
  ...
  public String toString() {
    return String.format(
        "%d:%d-%d:%d",
        mChunkStart >> 16, mChunkStart & 0xFFFF, mChunkEnd >> 16, mChunkEnd & 0xFFFF);
  }
