...
/**
 * Embodies defaults for global values that affect how the SAM JDK operates. Defaults are encoded in
 * the class and are also overridable using system properties.
 *
 * @author Tim Fennell
 */
public class Defaults {
  ...
  /**
   * Should BAM index files be created when writing out coordinate sorted BAM files? Default =
   * false.
   */
  ...
  /**
   * Should asynchronous read I/O be used where supported by the samtools package (one thread per
   * file). Default = false.
   */
  public static final boolean USE_ASYNC_IO_READ_FOR_SAMTOOLS;

  /**
   * Should asynchronous write I/O be used where supported by the samtools package (one thread per
   * file). Default = false.
   */
  public static final boolean USE_ASYNC_IO_WRITE_FOR_SAMTOOLS;

  /**
   * Should asynchronous write I/O be used where supported by the tribble package (one thread per
   * file). Default = false.
   */
  ...
  /**
   * Even if BUFFER_SIZE is 0, this is guaranteed to be non-zero. If BUFFER_SIZE is non-zero, this
   * == BUFFER_SIZE (Default = 128k).
   */
  public static final int NON_ZERO_BUFFER_SIZE;

  /**
   * The reference FASTA file. If this is not set, the file is null. This file may be required for
   * reading writing SAM files (ex. CRAM). Default = null.
   */
  public static final File REFERENCE_FASTA;

  /**
   * Custom reader factory able to handle URL based resources like ga4gh. Expected format: <url
   * prefix>,<fully qualified factory class name>[,<jar file name>] E.g.
   * https://www.googleapis.com/genomics/v1beta/reads/,com.google.genomics.ReaderFactory OR
   * https://www.googleapis.com/genomics/v1beta/reads/,com.google.genomics.ReaderFactory,/tmp/genomics.jar
   * Default = "".
   */
  public static final String CUSTOM_READER_FACTORY;

  /**
   * Boolean describing whether downloading a reference file is allowed (for CRAM files), in case
   * the reference file is not specified by the user Enabling this is not necessarily a good idea,
   * since this process often fails. Default = false.
   */
  public static final boolean USE_CRAM_REF_DOWNLOAD;

  /**
   * A mask (pattern) to use when building EBI reference service URL for a given MD5 checksum. Must
   * contain one and only one string placeholder. Default = "https://www.ebi.ac.uk/ena/cram/md5/%s".
   */
  public static final String EBI_REFERENCE_SERVICE_URL_MASK;

  /**
   * Boolean describing whether downloading of SRA native libraries is allowed, in case such native
   * libraries are not found locally. Default = false.
   */
  ...
  static {
    ...
    SAM_FLAG_FIELD_FORMAT =
        SamFlagField.valueOf(
            getStringProperty("sam_flag_field_format", SamFlagField.DECIMAL.name()));
  ...
  /**
   * Returns a map of all default values (keys are names), lexicographically sorted by keys. The
   * returned map is unmodifiable. This function is useful for example when logging all defaults.
   */
  ...
  /**
   * Gets a string system property, prefixed with "samjdk." using the default if the property does
   * not exist or if the java.security manager raises an exception for applications started with
  ...
  private static String getStringProperty(final String name, final String def) {
    try {
      ...
      log.warn(
          error,
          "java Security Manager forbids 'System.getProperty(\""
              + name
  ...
  /**
   * Checks whether a string system property, prefixed with "samjdk.", exists. If the property does
   * not exist or if the java.security manager raises an exception for applications started with
   * -Djava.security.manager this method returns false.
   */
  private static boolean hasProperty(final String name) {
    try {
      ...
      log.warn(
          error,
          "java Security Manager forbids 'System.getProperty(\"" + name + "\")' , returning false");
      return false;
    }
  }

  /**
   * Gets a boolean system property, prefixed with "samjdk." using the default if the property does
   * not exist.
   */
  ...
  /**
   * Gets an int system property, prefixed with "samjdk." using the default if the property does not
   * exist.
   */
  ...
  /**
   * Gets a File system property, prefixed with "samjdk." using the default if the property does not
   * exist.
   */
