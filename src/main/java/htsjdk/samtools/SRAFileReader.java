...
public class SRAFileReader extends SamReader.ReaderImplementation implements SamReader.Indexing {
  ...
  public SRAFileReader(final SRAAccession acc) {
    ...
    if (!acc.isValid()) {
      throw new IllegalArgumentException(
          "SRAFileReader: cannot resolve SRA accession '"
              + acc
              + "'\n"
  ...
  public CloseableIterator<SAMRecord> query(QueryInterval[] intervals, boolean contained) {
    ...
    for (QueryInterval interval : intervals) {
      ...
      if (!contained) {
        intervalSpan =
            index.getSpanOverlapping(interval.referenceIndex, interval.start, interval.end);
  ...
  public CloseableIterator<SAMRecord> queryAlignmentStart(String sequence, int start) {
    ...
    if (sequenceIndex == -1) {
      throw new IllegalArgumentException(
          "Unknown sequence '" + sequence + "' was passed to SRAFileReader");
    }
  ...
  public CloseableIterator<SAMRecord> queryUnmapped() {
    ...
    SAMFileSpan span =
        new BAMFileSpan(
            new Chunk(
                recordRangeInfo.getTotalReferencesLength(),
                recordRangeInfo.getTotalRecordRangeLength()));
    return getIterator(span);
  }
  ...
  /**
   * Returns true if the supported index is browseable, meaning the bins in it can be traversed and
   * chunk data inspected and retrieved.
   *
