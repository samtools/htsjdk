 ...
 *
 * <p>Here is how it works: SRA allows reading of alignments by Reference position fast, so we
 * divide our "file" range for alignments as a length of all references. Reading unaligned reads is
 * then fast if we use read positions for lookup and (internally) filter out aligned fragments.
 *
 * <p>Total SRA "file" range is calculated as sum of all reference lengths plus number of reads
 * (both aligned and unaligned) in SRA archive.
 *
 * <p>Now, we can use Chunks to lookup for aligned and unaligned fragments.
 *
 * <p>We emulate BAM index bins by mapping SRA reference positions to bin numbers. And then we map
 * from bin number to list of chunks, which represent SRA "file" positions (which are simply
 * reference positions).
 *
 * <p>We only emulate last level of BAM index bins (and they refer to a portion of reference
 * SRA_BIN_SIZE bases long). For all other bins RuntimeException will be returned (but since nobody
 * else creates bins, except SRAIndex class that is fine).
 *
 * <p>But since the last level of bins was not meant to refer to fragments that only partially
 * overlap bin reference positions, we also return chunk that goes 5000 bases left before beginning
 * of the bin to assure fragments that start before the bin positions but still overlap with it can
 * be retrieved by SRA reader. Later we will add support to NGS API to get a maximum number of bases
 * that we need to go left to retrieve such fragments.
 *
 * <p>Created by andrii.nikitiuk on 9/4/15.
 */
public class SRAIndex implements BrowseableBAMIndex {
  ...
  /**
   * How many bases should we go left on the reference to find all fragments that start before
   * requested interval but overlap with it
   */
  ...
  public BinList getBinsOverlapping(int referenceIndex, int startPos, int endPos) {
    ...
    binBitSet.set(
        SRA_BIN_INDEX_OFFSET + firstBinNumber, SRA_BIN_INDEX_OFFSET + lastBinNumber + 1, true);
    if (lastBinNumber + 1 < numberOfBins) {
      binBitSet.set(
          SRA_BIN_INDEX_OFFSET + lastBinNumber + 1, SRA_BIN_INDEX_OFFSET + numberOfBins, false);
    }
  ...
  public BAMIndexMetaData getMetaData(int reference) {
    throw new UnsupportedOperationException(
        "Getting of BAM index metadata for SRA is not implemented");
  }
  ...
  private List<Chunk> getBinChunks(Bin bin) {
    ...
    // move requested position MAX_FRAGMENT_OVERLAP bases behind, so that we take all the reads that
    // overlap requested position
