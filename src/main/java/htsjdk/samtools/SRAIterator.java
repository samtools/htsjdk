...
public class SRAIterator implements SAMRecordIterator {
  ...
  public static RecordRangeInfo getRecordsRangeInfo(ReadCollection run) {
    try {
      return new RecordRangeInfo(
          SRAUtils.getReferencesLengthsAligned(run), SRAUtils.getNumberOfReads(run));
    } catch (ErrorMsg e) {
   ...
   * @param cachedReferences list of cached references shared among all iterators from a single
   *     SRAFileReader
  ...
  /**
   * NGS iterators implement a single method "nextObject" which return true if the operation was
   * successful or false if there are no more objects available. That means that there is no way to
   * check "hasNext" without actually moving the iterator forward. Because of that all the logic of
   * moving iterator forward is actually happens in "hasNext".
   *
   * <p>Here is explanation of how it works: Iterator holds a list of chunks of requested records.
   * Here we have chunksIterator that walks though that list. We walk though that list using
   * chunksIterator. If current chunk can represent aligned fragments then we create
   * SRAAlignmentIterator iterator, pass the chunk into it and ask if it can find any record. If
   * record was found, we say that we have next; otherwise we check if the chunk can represent
   * unaligned fragments and then create SRAUnalignmentIterator if so and do the same steps as with
   * alignemnt iterator.
   *
   * <p>If record was not found in both SRAAlignmentIterator and SRAUnalignmentIterator (it is
   * possible that reference range has no alignments or that reads range has all aligned fragment),
   * we try the next chunk.
   *
   * <p>When there are no more chunks and both iterators have no more records we return false.
   *
  ...
  /**
   * Call hasNext to make sure that one of inner iterators points to the next record, the retrieve
   * the record from one of them.
   *
