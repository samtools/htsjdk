...
public class SamFiles {
  ...
  /**
   * Finds the index file associated with the provided SAM file. The index file must exist and be
   * reachable to be found.
   *
   * <p>If the file is a symlink and the index cannot be found, try to unsymlink the file and look
   * for the bai in the actual file path.
   *
  ...
  private static Path unsymlinkAndLookForIndex(Path samPath) {
    try {
      ...
      if (canonicalIndexPath != null) {
        LOG.warn(
            "The index file "
                + canonicalIndexPath.toAbsolutePath()
                + " was found by resolving the canonical path of a symlink: "
                + samPath.toAbsolutePath()
                + " -> "
                + samPath.toRealPath());
      }
  ...
  private static Path lookForIndex(final Path samPath) { // If input is foo.bam, look for foo.bai
    ...
    final String fileName =
        samPath.getFileName().toString(); // works for all path types (e.g. HDFS)
    if (fileName.endsWith(BamFileIoUtils.BAM_FILE_EXTENSION)) {
      final String bai =
          fileName.substring(0, fileName.length() - BamFileIoUtils.BAM_FILE_EXTENSION.length())
      ...
      final String crai =
          fileName.substring(0, fileName.length() - CramIO.CRAM_FILE_EXTENSION.length())
              + CRAIIndex.CRAI_INDEX_SUFFIX;
