...
public class SamFiles {
    ...
    private static Path lookForIndex(final Path samPath) {// If input is foo.bam, look for foo.bai or foo.csi
        ...
        if (fileName.endsWith(BamFileIoUtils.BAM_FILE_EXTENSION)) {
            ...
            final String csi = fileName.substring(0, fileName.length() - BamFileIoUtils.BAM_FILE_EXTENSION.length()) + BAMIndex.BAMIndexSuffix2;
        ...
        if (Files.isRegularFile(indexPath)) {
            ...
            indexPath = samPath.resolveSibling(fileName + BAMIndex.BAMIndexSuffix2);
            if (Files.isRegularFile(indexPath)) {
                return indexPath;
            }
        }
