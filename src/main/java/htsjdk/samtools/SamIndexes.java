...
/**
 * A helper class to read BAI and CRAI indexes. Main goal is to provide BAI stream as a sort of
 * common API for all index types.
 *
 * <p>Created by vadim on 14/08/2015.
 */
public enum SamIndexes {
  ...
  public static InputStream openIndexFileAsBaiOrNull(
      final File file, final SAMSequenceDictionary dictionary) throws IOException {
    return openIndexUrlAsBaiOrNull(file.toURI().toURL(), dictionary);
  }

  public static InputStream openIndexUrlAsBaiOrNull(
      final URL url, final SAMSequenceDictionary dictionary) throws IOException {
  ...
  public static InputStream asBaiStreamOrNull(
      final InputStream inputStream, final SAMSequenceDictionary dictionary) throws IOException {
  ...
  private static boolean doesStreamStartWith(final InputStream is, final byte[] bytes)
      throws IOException {
