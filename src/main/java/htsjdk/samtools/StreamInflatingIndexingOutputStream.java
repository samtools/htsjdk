...
/**
 * OutputStream implementation that writes output to an underlying output stream while also copying
 * the same bytes to a PipedOutputStream that routes the data back into an Indexer to generate a
 * BAMIndex by inflating and decoding the stream and feeding the SAMRecords to a BAMIndexer.
 */
...
/**
 * A little class that takes an InputStream from which it reads a BAM file, generates a BAMIndex and
 * then writes the index to the Path provided. All operations are designed to be carried out in a
 * separate thread.
 */
class Indexer implements Runnable {
  ...
  /**
   * Constructs an indexer that reads from the stream provided and writes an index to the Path
   * provided.
   */
