...
/**
 * A class representing CRAI index entry: file and alignment offsets for each slice. Created by
 * vadim on 10/08/2015.
 */
  ...
  public CRAIEntry(final String line) throws CRAIIndex.CRAIIndexException {
    ...
    if (chunks.length != CRAI_INDEX_COLUMNS) {
      throw new CRAIIndex.CRAIIndexException(
          "Malformed CRAI index entry: expecting "
              + CRAI_INDEX_COLUMNS
              + " columns but got "
              + chunks.length);
    }
  ...
  public static Comparator<CRAIEntry> byStartDesc =
      new Comparator<CRAIEntry>() {
        ...
        public int compare(CRAIEntry o1, CRAIEntry o2) {
          if (o1.sequenceId != o2.sequenceId) {
            if (o1.sequenceId == SAMRecord.NO_ALIGNMENT_REFERENCE_INDEX) return 1;
            if (o2.sequenceId == SAMRecord.NO_ALIGNMENT_REFERENCE_INDEX) return -1;
            return -o2.sequenceId + o1.sequenceId;
          }
          if (o1.alignmentStart != o2.alignmentStart) return o1.alignmentStart - o2.alignmentStart;
