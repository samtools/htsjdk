...
public class CRAIIndex {
  ...
  /**
   * Return a list of CRAI Entries; one for each reference in the multireference slice. TODO: this
   * should be refactored and delegate to container/slice
   */
  private static Collection<CRAIEntry> getCRAIEntriesForMultiRefSlice(
    ...
    final RefSeqIdReader reader =
        new RefSeqIdReader(
            slice.sequenceId, slice.alignmentStart, ValidationStringency.DEFAULT_STRINGENCY);
  ...
  public static SeekableStream openCraiFileAsBaiStream(
      final File cramIndexFile, final SAMSequenceDictionary dictionary) throws IOException {
  ...
  public static SeekableStream openCraiFileAsBaiStream(
      ...
      throws IOException, CRAIIndexException {
  ...
  public static List<CRAIEntry> find(
      final List<CRAIEntry> list, final int seqId, final int start, final int span) {
  ...
  /**
   * Find index of the last aligned entry in the list. Assumes the index is sorted by coordinate and
   * unmapped entries (with sequence id = -1) follow the mapped entries.
   *
