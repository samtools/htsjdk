...
/**
 * A class responsible for decisions about which encodings to use for a given set of records. This
 * particular version relies heavily on GZIP and RANS for better compression.
 */
public class CompressionHeaderFactory {
   ...
   * @param substitutionMatrix a matrix of base substitution frequencies, can be null, in which case
   *     it is re-calculated.
   * @param sorted if true the records are assumed to be sorted by alignment position
   * @return {@link htsjdk.samtools.cram.structure.CompressionHeader} object describing the encoding
   *     chosen for the data
   */
  ...
  /**
   * Iterate over the records and for each tag found come up with an encoding. Tag encodings are
  ...
  private void buildTagEncodings(
      final List<CramCompressionRecord> records, final CompressionHeaderBuilder builder) {
  ...
  static void updateSubstitutionCodes(
      final List<CramCompressionRecord> records, final SubstitutionMatrix substitutionMatrix) {
    for (final CramCompressionRecord record : records) {
   ...
   * @return a 2D array of frequencies, see {@link
  ...
  private static byte[][][] buildTagIdDictionary(final List<CramCompressionRecord> records) {
    final Comparator<ReadTag> comparator =
        new Comparator<ReadTag>() {
    ...
    final Comparator<byte[]> baComparator =
        new Comparator<byte[]>() {
  ...
  /**
   * Tag id is and integer where the first byte is its type and the other 2 bytes represent the
   * name. For example 'OQZ', where 'OQ' stands for original quality score tag and 'Z' stands for
   * string type.
   *
  ...
  static ByteSizeRange geByteSizeRangeOfTagValues(
    ...
    for (final CramCompressionRecord record : records) {
      ...
      for (final ReadTag tag : record.tags) {
        ...
        if (stats.min > size) stats.min = size;
        if (stats.max < size) stats.max = size;
      }
    }
    return stats;
  }
  ...
  /**
   * A combination of external compressor and encoding params. This is all that is needed to encode
   * a data series.
   */
   ...
   */
  private EncodingDetails buildEncodingForTag(
      final List<CramCompressionRecord> records, final int tagID) {
    ...
    switch (type) {
      ...
      case 'B':
        ...
        details.params =
            ByteArrayLenEncoding.toParam(
                ExternalIntegerEncoding.toParam(tagID), ExternalByteEncoding.toParam(tagID));
        return details;
      default:
  ...
  private static class CompressionHeaderBuilder {
    ...
    void addExternalEncoding(
        final EncodingKey encodingKey,
    ...
    void addExternalByteArrayStopTabGzipEncoding(final EncodingKey encodingKey) {
      addExternalEncoding(
          encodingKey,
          ByteArrayStopEncoding.toParam((byte) '\t', encodingKey.ordinal()),
          ExternalCompressor.createGZIP());
    }

    void addExternalIntegerEncoding(
        final EncodingKey encodingKey, final ExternalCompressor compressor) {
      addExternalEncoding(
          encodingKey, ExternalIntegerEncoding.toParam(encodingKey.ordinal()), compressor);
    }

    void addExternalIntegerGzipEncoding(final EncodingKey encodingKey) {
      addExternalEncoding(
          encodingKey,
          ExternalIntegerEncoding.toParam(encodingKey.ordinal()),
          ExternalCompressor.createGZIP());
    }
    ...
    void addExternalByteGzipEncoding(final EncodingKey encodingKey) {
      addExternalEncoding(
          encodingKey,
          ExternalByteEncoding.toParam(encodingKey.ordinal()),
          ExternalCompressor.createGZIP());
    }
  ...
  static int getTagValueByteSize(final byte type, final Object value) {
    switch (type) {
      ...
      case 'B':
        if (value instanceof byte[]) return 1 + 4 + ((byte[]) value).length;
        if (value instanceof short[]) return 1 + 4 + ((short[]) value).length * 2;
        if (value instanceof int[]) return 1 + 4 + ((int[]) value).length * 4;
        if (value instanceof float[]) return 1 + 4 + ((float[]) value).length * 4;
        if (value instanceof long[]) return 1 + 4 + ((long[]) value).length * 4;
