...
public class Cram2SamRecordFactory {
  ...
  public SAMRecord create(final CramCompressionRecord cramRecord) {
    ...
    if (cramRecord.isSegmentUnmapped()) samRecord.setCigarString(SAMRecord.NO_ALIGNMENT_CIGAR);
    else samRecord.setCigar(getCigar2(cramRecord.readFeatures, cramRecord.readLength));

    if (samRecord.getReadPairedFlag()) {
      samRecord.setMateReferenceIndex(cramRecord.mateSequenceID);
      samRecord.setMateAlignmentStart(
          cramRecord.mateAlignmentStart > 0
    ...
    if (cramRecord.readGroupID > -1) {
      final SAMReadGroupRecord readGroupRecord = header.getReadGroups().get(cramRecord.readGroupID);
  ...
  private static Cigar getCigar2(final Collection<ReadFeature> features, final int readLength) {
    ...
    for (final ReadFeature feature : features) {
      ...
      if (!cigarOperator.consumesReadBases()) lastOpPos -= readFeatureLength;
    }
    ...
    if (lastOperator != null) {
      if (lastOperator != CigarOperator.M) {
        ...
        if (readLength >= lastOpPos + lastOpLen) {
          cigarElement =
              new CigarElement(readLength - (lastOpLen + lastOpPos) + 1, CigarOperator.M);
          list.add(cigarElement);
        }
      } else if (readLength == 0 || readLength > lastOpPos - 1) {
        if (readLength == 0) cigarElement = new CigarElement(lastOpLen, CigarOperator.M);
        else cigarElement = new CigarElement(readLength - lastOpPos + 1, CigarOperator.M);
        list.add(cigarElement);
      }
    }
