...
public class CramIO {
  ...
  /**
   * The 'zero-B' EOF marker as per CRAM specs v2.1. This is basically a serialized empty CRAM
   * container with sequence id set to some number to spell out 'EOF' in hex.
   */
  public static final byte[] ZERO_B_EOF_MARKER =
      bytesFromHex(
          "0b 00 00 00 ff ff ff ff ff e0 45 4f 46 00 00 00 00 01 00 00 01 00 06 06 01 00 "
              ...
              + "01 00 01 00");
  /**
   * The zero-F EOF marker as per CRAM specs v3.0. This is basically a serialized empty CRAM
   * container with sequence id set to some number to spell out 'EOF' in hex.
   */
  public static final byte[] ZERO_F_EOF_MARKER =
      bytesFromHex(
          "0f 00 00 00 ff ff ff ff 0f e0 45 4f 46 00 00 00 00 01 00 05 bd d9 4f 00 01 00 "
              + ""
  ...
  private static boolean streamEndsWith(final SeekableStream seekableStream, final byte[] marker)
      throws IOException {
  ...
  private static boolean checkEOF(final Version version, final SeekableStream seekableStream)
      throws IOException {
  ...
  /**
   * Check if the file: 1) contains proper CRAM header. 2) given the version info from the header
   * check the end of file marker.
   *
   * @param file the CRAM file to check
   * @return true if the file is a valid CRAM file and is properly terminated with respect to the
   *     version.
   * @throws IOException as per java IO contract
   */
  ...
  public static long writeCramHeader(final CramHeader cramHeader, final OutputStream outputStream)
      throws IOException {
    //        if (cramHeader.getVersion().major < 3) throw new RuntimeException("Deprecated CRAM
    // version: " + cramHeader.getVersion().major);
    ...
    for (int i = cramHeader.getId().length; i < 20; i++) outputStream.write(0);

    final long length =
        CramIO.writeContainerForSamFileHeader(
            cramHeader.getVersion().major, cramHeader.getSamFileHeader(), outputStream);
  ...
  public static CramHeader readCramHeader(final InputStream inputStream) throws IOException {
    ...
    final SAMFileHeader samFileHeader =
        readSAMFileHeader(header.getVersion(), inputStream, new String(header.getId()));
  ...
  private static long writeContainerForSamFileHeader(
      final int major, final SAMFileHeader samFileHeader, final OutputStream os)
      throws IOException {
  ...
  private static SAMFileHeader readSAMFileHeader(
      final Version version, InputStream inputStream, final String id) throws IOException {
    ...
    for (int i = 0; i < 4; i++) buffer.put((byte) inputStream.read());
    buffer.flip();
    ...
    final BufferedLineReader bufferedLineReader =
        new BufferedLineReader(new ByteArrayInputStream(bytes));
    final SAMTextHeaderCodec codec = new SAMTextHeaderCodec();
    return codec.decode(bufferedLineReader, id);
  }

  /**
   * Attempt to replace the SAM file header in the CRAM file. This will succeed only if there is
   * sufficient space reserved in the existing CRAM header. The implementation re-writes the first
   * FILE_HEADER block in the first container of the CRAM file using random file access.
   *
  ...
  public static boolean replaceCramHeader(final File file, final CramHeader newHeader)
    ...
    final CountingInputStream countingInputStream =
        new CountingInputStream(new FileInputStream(file));

    final CramHeader header = readFormatDefinition(countingInputStream);
    final Container c =
        ContainerIO.readContainerHeader(header.getVersion().major, countingInputStream);
