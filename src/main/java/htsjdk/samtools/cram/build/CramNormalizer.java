...
public class CramNormalizer {
  ...
  public void normalize(
      final ArrayList<CramCompressionRecord> records,
      final byte[] ref,
    ...
    { // restore pairing first:
      for (final CramCompressionRecord record : records) {
        ...
        if (record.isHasMateDownStream()) {
          final CramCompressionRecord downMate =
              records.get(record.index + record.recordsToNextFragment - startCounter);
    ...
    for (final CramCompressionRecord record : records) {
      if (record.readName == null) {
        ...
        if (record.next != null) record.next.readName = name;
        if (record.previous != null) record.previous.readName = name;
      }
    }
    ...
    for (final CramCompressionRecord record : records) {
      ...
      if (record.isUnknownBases()) {
        ...
        record.readBases =
            restoreReadBases(record, refBases, refOffset_zeroBased, substitutionMatrix);
    }
  ...
  private static void setNextMate(
      final CramCompressionRecord record, final CramCompressionRecord next) {
  ...
  public static void restoreQualityScores(
      final byte defaultQualityScore, final List<CramCompressionRecord> records) {
    for (final CramCompressionRecord record : records)
      restoreQualityScores(defaultQualityScore, record);
  }

  private static byte[] restoreQualityScores(
      final byte defaultQualityScore, final CramCompressionRecord record) {
    if (!record.isForcePreserveQualityScores()) {
      ...
      if (star) record.qualityScores = SAMRecord.NULL_QUALS;
      else record.qualityScores = scores;
    } else {
      ...
      if (missingScores == scores.length) record.qualityScores = SAMRecord.NULL_QUALS;
    }

    return record.qualityScores;
  }

  private static byte[] restoreReadBases(
    ...
    if (record.readFeatures == null || record.readFeatures.isEmpty()) {
      if (ref.length + refOffsetZeroBased < alignmentStart + bases.length) {
        ...
        System.arraycopy(
            ...
            bases,
            0,
            Math.min(bases.length, ref.length + refOffsetZeroBased - alignmentStart));
      } else System.arraycopy(ref, alignmentStart - refOffsetZeroBased, bases, 0, bases.length);
    ...
    for (final ReadFeature variation : variations) {
      ...
      switch (variation.getOperator()) {
        case Substitution.operator:
          ...
          byte refBase =
              getByteOrDefault(ref, alignmentStart + posInSeq - refOffsetZeroBased, (byte) 'N');
    ...
    for (;
        posInRead <= readLength && alignmentStart + posInSeq - refOffsetZeroBased < ref.length;
        posInRead++, posInSeq++) {
  ...
  private static byte getByteOrDefault(
      final byte[] array, final int pos, final byte outOfBoundsValue) {
    if (pos >= array.length) return outOfBoundsValue;
    else return array[pos];
  }

  /**
   * The method is similar in semantics to {@link
   * htsjdk.samtools.SamPairUtil#computeInsertSize(SAMRecord, SAMRecord) computeInsertSize} but
   * operates on CRAM native records instead of SAMRecord objects.
   *
   * @param firstEnd first mate of the pair
   * @param secondEnd second mate of the pair
   * @return template length
   */
  public static int computeInsertSize(
      final CramCompressionRecord firstEnd, final CramCompressionRecord secondEnd) {
    ...
    final int firstEnd5PrimePosition =
        firstEnd.isNegativeStrand() ? firstEnd.getAlignmentEnd() : firstEnd.alignmentStart;
    final int secondEnd5PrimePosition =
        secondEnd.isNegativeStrand() ? secondEnd.getAlignmentEnd() : secondEnd.alignmentStart;
