 ...
 * <p>
 *
 * <p>Note that this class is for internal optimization purposes only, and may not be supported in
 * future releases of Apache Commons Lang. Utilities of this sort may be included in future releases
...
public class IntHashMap<T> {
  ...
  /**
   * Constructs a new, empty hashtable with a default capacity and load factor, which is <code>20
   * </code> and <code>0.75</code> respectively.
   */
  public IntHashMap() {
    this(20, 0.75f);
  }
   ...
   * <p>
   *
   * <p>Note that this method is identical in functionality to contains (which predates the Map
   * interface).
   *
   ...
   * <p>
   *
   * <p>This method is called automatically when the number of keys in the hashtable exceeds this
   ...
   * <p>
   *
   * <p>This method does nothing if the key is not present in the hashtable.
   *
  ...
  public Object remove(final int key) {
    final Entry<T>[] tab = table;
    final int index = (key & 0x7FFFFFFF) % tab.length;
    for (Entry<T> entry = tab[index], prev = null;
        entry != null;
