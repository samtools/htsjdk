...
public class FQZParams {
    ...
    public FQZParams(final ByteBuffer inBuffer) {
        ...
        int maxSelector = numParamBlock > 1 ?
                numParamBlock : // TODO: code says numParamBlock, javascript says numParamBlock - 1
                0;
        if (fqzFlags.hasSelectorTable()) {
            maxSelector = inBuffer.get() & 0xFF;
            FQZUtils.readArray(inBuffer, selectorTable, 256); //TODO: the C code read_array returns an "unused" value ??
        } else {
