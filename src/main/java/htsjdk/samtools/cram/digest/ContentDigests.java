...
public class ContentDigests {
  ...
  public static ContentDigests create(final EnumSet<KNOWN_DIGESTS> requestedDigests) {
    ...
    for (final KNOWN_DIGESTS digest : requestedDigests) digesters.add(digest.createDigester());
  ...
  public static ContentDigests create(final SAMBinaryTagAndValue binaryTags) {
    ...
    while (binaryTag != null) {
      final String tagID = SAMTagUtil.getSingleton().makeStringTag(binaryTag.tag);
  ...
  void add(final SAMRecord record) {
    for (final Digester digester : digesters) digester.add(record);
  }

  public void add(final CramCompressionRecord record) {
    for (final Digester digester : digesters) digester.addCramRecord(record);
  }

  public void addSAMRecords(final Iterable<SAMRecord> records) {
    for (final SAMRecord record : records) add(record);
  }

  public void addCramRecords(final Iterable<CramCompressionRecord> records) {
    for (final CramCompressionRecord record : records) add(record);
  }

  public SAMBinaryTagAndValue getAsTags() {
    SAMBinaryTagAndValue tag = null;
    for (final Digester digester : digesters) {
      if (tag == null) tag = digester.toTag();
      else tag = tag.insert(digester.toTag());
    }
    ...
    for (final Digester digester : digesters) {
      ...
      if (!(foundTag.value instanceof byte[]))
        throw new RuntimeException(
            "Expecting a byte array but got: " + foundTag.value.getClass().getName());
      ...
      if (!Arrays.equals(expected, actual)) {
        ...
        log.error(
            String.format(
                "Content hash mismatch for tag %s, actual: %s; expected: %s",
                digester.tagID, actualString, expectedString));
        return false;
      } else log.debug("Content digest ok: " + digester.tagID);
    }
    return true;
  }
  ...
  public enum KNOWN_DIGESTS {
    ...
    B5 {
      ...
      Digester createDigester() {
        try {
          return new Digester(
              new MessageDigestHasher(
                  MessageDigest.getInstance("SHA-512"), new ByteSumCombine(), null),
              SERIES.BASES,
              name());
    ...
    B1 {
      ...
      Digester createDigester() {
        try {
          return new Digester(
              new MessageDigestHasher(
                  MessageDigest.getInstance("SHA-1"), new ByteSumCombine(), null),
              SERIES.BASES,
              name());
        } catch (final NoSuchAlgorithmException e) {
          throw new RuntimeException(e);
