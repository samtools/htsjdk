 ...
 * License inputStream distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY
...
public class ByteArrayStopEncoding implements Encoding<byte[]> {
  ...
  public static EncodingParams toParam(final byte stopByte, final int externalId) {
    final ByteArrayStopEncoding e = new ByteArrayStopEncoding(stopByte, externalId);
  ...
  @Override
  public BitCodec<byte[]> buildCodec(
    ...
    final ExposedByteArrayOutputStream os = outputMap == null ? null : outputMap.get(externalId);
  ...
  public static class ByteArrayStopCodec extends AbstractBitCodec<byte[]> {
    ...
    public ByteArrayStopCodec(
        final byte stopByte, final InputStream inputStream, final OutputStream outputStream) {
    ...
    public byte[] read(final BitInputStream bitInputStream) throws IOException {
      readingBAOS.reset();
      while ((b = inputStream.read()) != -1 && b != stop) readingBAOS.write(b);
