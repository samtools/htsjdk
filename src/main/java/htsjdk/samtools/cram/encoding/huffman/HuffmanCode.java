...
public class HuffmanCode {
  ...
  public static <T> HuffmanTree<T> buildTree(final int[] charFrequencies, final T[] values) {
    ...
    for (int i = 0; i < charFrequencies.length; i++)
      if (charFrequencies[i] > 0) list.add(new HuffmanLeaf<T>(charFrequencies[i], values[i]));

    final Comparator<HuffmanTree<T>> comparator =
        new Comparator<HuffmanTree<T>>() {
  ...
  public static <T> void getValuesAndBitLengths(
      final List<T> values, final List<Integer> lens, final HuffmanTree<T> tree) {
  ...
  private static <T> void getBitCode(
      final HuffmanTree<T> tree,
      final HuffmanBitCode<T> code,
      final Map<T, HuffmanBitCode<T>> codes) {
