...
public class HuffmanByteEncoding implements Encoding<Byte> {
  ...
  public byte[] toByteArray() {
    buf.clear();
    if (buf.capacity() < values.length * 8) buf = ByteBuffer.allocate(values.length * 8);

    ITF8.writeUnsignedITF8(values.length, buf);
    for (final byte value : values) buf.put(value);

    ITF8.writeUnsignedITF8(bitLengths.length, buf);
    for (final int value : bitLengths) ITF8.writeUnsignedITF8(value, buf);
  ...
  public void fromByteArray(final byte[] data) {
    ...
    for (int i = 0; i < size; i++) bitLengths[i] = ITF8.readUnsignedITF8(buf);
  }
