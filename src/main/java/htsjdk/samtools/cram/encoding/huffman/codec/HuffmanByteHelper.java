...
class HuffmanByteHelper {
  ...
  HuffmanByteHelper(final byte[] values, final int[] bitLengths) {
    ...
    for (int i = 0; i < values.length; i++) this.values[i] = 0xFF & values[i];
    ...
    final ArrayList<HuffmanBitCode> list = new ArrayList<HuffmanBitCode>(codes.size());
    ...
    for (int i = 0; i < sortedCodes.length; i++) {
      ...
      if (maxBitCode < sortedCodes[i].bitCode) maxBitCode = sortedCodes[i].bitCode;
    }
  ...
  private void buildCodeBook() {
    ...
    for (int i = 0; i < values.length; i++) {
      if (codeBook.containsKey(bitLengths[i])) codeBook.get(bitLengths[i]).add(values[i]);
      else {
  ...
  private void buildCodes() {
    ...
    for (final Object key : codeBook.keySet()) { // Iterate over code lengths
      ...
      final SortedSet<Integer> get = codeBook.get(key);
  ...
  final long write(final BitOutputStream bitOutputStream, final byte value) throws IOException {
    final HuffmanBitCode code = valueToCode[value];
    if (code.value != value)
      throw new RuntimeException(
          String.format("Searching for %d but found %s.", value, code.toString()));
  ...
  final byte read(final BitInputStream bitInputStream) throws IOException {
    ...
    for (int i = 0; i < sortedCodes.length; i++) {
      ...
      for (int j = i; sortedCodes[j + 1].bitLength == length && j < sortedCodes.length; j++) i++;
    }

    throw new RuntimeException("Not found.");
  }

  private static final Comparator<HuffmanBitCode> bitCodeComparator =
      new Comparator<HuffmanBitCode>() {

        @Override
        public int compare(final HuffmanBitCode o1, final HuffmanBitCode o2) {
          final int result = o1.bitLength - o2.bitLength;
          if (result == 0) return o1.bitCode - o2.bitCode;
          else return result;
        }
      };
