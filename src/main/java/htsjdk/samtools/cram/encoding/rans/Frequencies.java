...
class Frequencies {
  ...
  static void readStats_o0(
      final ByteBuffer cp, final Decoding.AriDecoder decoder, final Decoding.RansDecSymbol[] syms) {
    ...
    do {
      ...
      if (decoder.R == null) decoder.R = new byte[Constants.TOTFREQ];
  ...
  static void readStats_o1(
      final ByteBuffer cp, final Decoding.AriDecoder[] D, final Decoding.RansDecSymbol[][] syms) {
    int rle_i = 0;
    int i = 0xFF & cp.get();
    do {
      int rle_j = 0;
      ...
      do {
        ...
        /* Build reverse lookup table */
        if (D[i].R == null) D[i].R = new byte[Constants.TOTFREQ];
  ...
  static int[] calcFrequencies_o0(final ByteBuffer in) {
    ...
    fsum++;
    if (fsum < Constants.TOTFREQ) F[M] += Constants.TOTFREQ - fsum;
    else F[M] -= fsum - Constants.TOTFREQ;
  ...
  static int[][] calcFrequencies_o1(final ByteBuffer in) {
    ...
    for (int i = 0; i < 256; i++) {
      ...
      t2++;
      if (t2 < Constants.TOTFREQ) F[i][M] += Constants.TOTFREQ - t2;
      else F[i][M] -= t2 - Constants.TOTFREQ;
    }
  ...
  static RansEncSymbol[] buildSyms_o0(final int[] F) {
    ...
    for (int i = 0; i < syms.length; i++) syms[i] = new RansEncSymbol();
  ...
  static RansEncSymbol[][] buildSyms_o1(final int[][] F) {
    ...
    for (int i = 0; i < syms.length; i++)
      for (int j = 0; j < syms[i].length; j++) syms[i][j] = new RansEncSymbol();
  ...
  static int writeFrequencies_o1(final ByteBuffer cp, final int[][] F) {
    ...
    for (int i = 0; i < 256; i++) for (int j = 0; j < 256; j++) T[i] += F[i][j];
