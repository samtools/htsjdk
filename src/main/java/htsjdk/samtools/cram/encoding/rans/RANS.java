...
public class RANS {
  ...
  private static final int PREFIX_BYTE_LENGTH =
  ...
  public static ByteBuffer uncompress(final ByteBuffer in, ByteBuffer out) {
    if (in.remaining() == 0) return ByteBuffer.allocate(0);
    ...
    if (out == null) out = ByteBuffer.allocate(out_sz);
    else out.limit(out_sz);
  ...
  public static ByteBuffer compress(final ByteBuffer in, final ORDER order, final ByteBuffer out) {
    if (in.remaining() == 0) return EMPTY_BUFFER;

    if (in.remaining() < 4) return encode_order0_way4(in, out);
  ...
  private static ByteBuffer allocateIfNeeded(final int in_size, final ByteBuffer out_buf) {
    final int compressedSize = (int) (1.05 * in_size + 257 * 257 * 3 + 4);
    if (out_buf == null) return ByteBuffer.allocate(compressedSize);
  ...
  private static ByteBuffer encode_order0_way4(final ByteBuffer in, ByteBuffer out_buf) {
    ...
    finalizeCompressed(0, out_buf, in_size, frequencyTable_size, compressedBlob_size);
    return out_buf;
  }

  private static ByteBuffer encode_order1_way4(final ByteBuffer in, ByteBuffer out_buf) {
    final int in_size = in.remaining();
    out_buf = allocateIfNeeded(in_size, out_buf);
    final int freqTableStart = PREFIX_BYTE_LENGTH;
    out_buf.position(freqTableStart);
    ...
    finalizeCompressed(1, out_buf, in_size, frequencyTable_size, compressedBlob_size);
    return out_buf;
  }

  private static void finalizeCompressed(
      final int order,
      final ByteBuffer out_buf,
      final int in_size,
      final int frequencyTable_size,
  ...
  private static ByteBuffer uncompress_order0_way4(final ByteBuffer in, final ByteBuffer out) {
    in.order(ByteOrder.LITTLE_ENDIAN);
    final Decoding.AriDecoder D = new Decoding.AriDecoder();
    final Decoding.RansDecSymbol[] syms = new Decoding.RansDecSymbol[256];
    for (int i = 0; i < syms.length; i++) syms[i] = new Decoding.RansDecSymbol();
  ...
  private static ByteBuffer uncompress_order1_way4(final ByteBuffer in, final ByteBuffer out_buf) {
    ...
    for (int i = 0; i < syms.length; i++)
      for (int j = 0; j < syms[i].length; j++) syms[i][j] = new Decoding.RansDecSymbol();
