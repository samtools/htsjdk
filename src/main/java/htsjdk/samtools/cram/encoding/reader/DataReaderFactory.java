...
public class DataReaderFactory {
  ...
  public AbstractReader buildReader(
    ...
    for (final Field field : reader.getClass().getFields()) {
      if (field.isAnnotationPresent(DataSeries.class)) {
        ...
        if (header.encodingMap.get(key) != null) {
          try {
            field.set(
                reader, createReader(type, header.encodingMap.get(key), bitInputStream, inputMap));
      ...
      if (field.isAnnotationPresent(DataSeriesMap.class)) {
        ...
        if ("TAG".equals(name)) {
          ...
          for (final Integer key : header.tMap.keySet()) {
            ...
            final DataReader<byte[]> tagReader =
                createReader(DataSeriesType.BYTE_ARRAY, params, bitInputStream, inputMap);
  ...
  private <T> DataReader<T> createReader(
    ...
    if (params.id == EncodingID.NULL)
      ...
      return collectStats
          ? new DataReaderWithStats(buildNullReader(valueType))
          : buildNullReader(valueType);
    ...
    if (encoding == null)
      throw new RuntimeException(
          "Encoding not found for value type " + valueType.name() + ", id=" + params.id);
    encoding.fromByteArray(params.params);

    //noinspection ConstantConditions
    return collectStats
        ? new DataReaderWithStats(
            new DefaultDataReader<T>(encoding.buildCodec(inputMap, null), bitInputStream))
        : new DefaultDataReader<T>(encoding.buildCodec(inputMap, null), bitInputStream);
  }
  ...
  private static <T> DataReader<T> buildNullReader(final DataSeriesType valueType) {
    switch (valueType) {
      ...
      case INT:
        return (DataReader<T>) new SingleValueReader<Integer>(0);
      case LONG:
  ...
  public Map<String, DataReaderWithStats> getStats(final CramRecordReader reader)
    ...
    final Map<String, DataReaderWithStats> map =
        new TreeMap<String, DataReaderFactory.DataReaderWithStats>();
    ...
    for (final Field field : reader.getClass().getFields()) {
      ...
      if (field.isAnnotationPresent(DataSeriesMap.class)) {
        ...
        final String name = dataSeriesMap.name();
        if ("TAG".equals(name)) {
          final Map<Integer, DataReader<byte[]>> tagMap =
              (Map<Integer, DataReader<byte[]>>) field.get(reader);
