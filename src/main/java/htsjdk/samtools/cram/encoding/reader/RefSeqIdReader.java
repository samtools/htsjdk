...
/**
 * A reader that only keeps track of alignment spans. The intended use is for CRAI index.
 *
 * @author vadim
 */
public class RefSeqIdReader extends AbstractReader {
  ...
  public RefSeqIdReader(
      final int seqId, final int alignmentStart, ValidationStringency validationStringency) {
    super();
  ...
  public void read() {
    ...
    try {
      ...
      if (refId == Slice.MULTI_REFERENCE) cramRecord.sequenceId = refIdCodec.readData();
    ...
    } else spans.get(cramRecord.sequenceId).addSingle(alignmentStart, cramRecord.readLength);
  }
}
