...
public class DataWriterFactory {
  ...
  public Writer buildWriter(
    ...
    for (final Field f : writer.getClass().getFields()) {
      if (f.isAnnotationPresent(DataSeries.class)) {
        ...
        f.set(writer, createWriter(type, h.encodingMap.get(key), bitOutputStream, outputMap));
      }
      ...
      if (f.isAnnotationPresent(DataSeriesMap.class)) {
        ...
        if ("TAG".equals(name)) {
          ...
          for (final Integer key : h.tMap.keySet()) {
            ...
            final DataWriter<byte[]> tagWriter =
                createWriter(DataSeriesType.BYTE_ARRAY, params, bitOutputStream, outputMap);
  ...
  private <T> DataWriter<T> createWriter(
    ...
    return new DefaultDataWriter<T>(encoding.buildCodec(null, outputMap), bitOutputStream);
  }
