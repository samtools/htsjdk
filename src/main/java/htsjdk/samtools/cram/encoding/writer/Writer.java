...
public class Writer {
  ...
  public void write(final CramCompressionRecord r) throws IOException {
    ...
    if (AP_delta) alStartC.writeData(r.alignmentDelta);
    else alStartC.writeData(r.alignmentStart);
    ...
    if (r.isDetached()) {
      ...
      if (!isCaptureReadNames()) readNameC.writeData(r.readName.getBytes(charset));
      ...
      detachedCount++;
    } else if (r.isHasMateDownStream()) distanceC.writeData(r.recordsToNextFragment);
    ...
    if (!r.isSegmentUnmapped()) {
          ...
          case Substitution.operator:
            ...
            if (sv.getCode() < 0)
              baseSubstitutionCodeCodec.writeData(
                  substitutionMatrix.code(sv.getReferenceBase(), sv.getBase()));
      ...
      if (!r.isUnknownBases()) for (final byte b : r.readBases) baseCodec.writeData(b);
      if (r.isForcePreserveQualityScores()) {
        qualityScoreArrayCodec.writeData(r.qualityScores);
      }
    }
