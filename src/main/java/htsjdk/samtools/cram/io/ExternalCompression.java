...
public class ExternalCompression {
  private static final int GZIP_COMPRESSION_LEVEL =
      Integer.valueOf(System.getProperty("gzip.compression.level", "5"));

  /**
   * Compress a byte array into GZIP blob. The method obeys {@link
   * ExternalCompression#GZIP_COMPRESSION_LEVEL} compression level.
   *
  ...
  public static byte[] xz(final byte[] data) throws IOException {
    ...
    final XZCompressorOutputStream xzCompressorOutputStream =
  ...
  public static byte[] unxz(final byte[] data) throws IOException {
    final XZCompressorInputStream xzCompressorInputStream =
