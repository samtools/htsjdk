...
public class LTF8 {
  /**
   * Reads an unsigned long value from the input stream. The sign bit should be interpreted just as
   * other bits in the value.
   *
  ...
  /**
   * Writes an unsigned long value to the output stream. The sign bit is interpreted just as other
   * bits in the value.
   *
   ...
   * @throws IOException as per java IO contract
   */
  public static int writeUnsignedLTF8(final long value, final OutputStream outputStream)
      throws IOException {
    if ((value >>> 7) == 0) {
      // no control bits
