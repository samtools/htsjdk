...
public class QualityScorePreservation {
  ...
  private static QualityScoreTreatment readTreatment(final LinkedList<Character> list) {
  ...
  private static List<PreservationPolicy> parsePolicies(final String spec) {
    ...
    Collections.sort(
        policyList,
        new Comparator<PreservationPolicy>() {
  ...
  private static PreservationPolicy parseSinglePolicy(final String spec) {
    ...
    for (final char character : spec.toCharArray()) list.add(character);
  ...
  private static void applyBinning(final byte[] scores) {
    for (int i = 0; i < scores.length; i++) scores[i] = Binning.ILLUMINA_BINNING_MATRIX[scores[i]];
  }

  private static byte applyTreatment(
      final byte score, final QualityScoreTreatment qualityScoreTreatment) {
    ...
    throw new RuntimeException(
        "Unknown quality score treatment type: " + qualityScoreTreatment.type.name());
  }

  public void addQualityScores(
      final SAMRecord samRecord,
      final CramCompressionRecord cramRecord,
  ...
  private static final Comparator<ReadFeature> readFeaturePositionComparator =
      new Comparator<ReadFeature>() {
  ...
  private static void addQS(
      final SAMRecord samRecord,
      final CramCompressionRecord cramRecord,
      final byte[] scores,
      final ReferenceTracks referenceTracks,
      final PreservationPolicy preservationPolicy) {
    ...
    if (preservationPolicy.readCategory != null) {
      ...
      switch (preservationPolicy.readCategory.type) {
        ...
        default:
          throw new RuntimeException(
              "Unknown read category: " + preservationPolicy.readCategory.type.name());
      }
    ...
    if (preservationPolicy.baseCategories == null || preservationPolicy.baseCategories.isEmpty()) {
      switch (preservationPolicy.treatment.type) {
        ...
        default:
          throw new RuntimeException(
              "Unknown quality score treatment type: " + preservationPolicy.treatment.type.name());
      }
      ...
      switch (baseCategory.type) {
        case FLANKING_DELETION:
          ...
          for (final CigarElement cigarElement : samRecord.getCigar().getCigarElements()) {
            if (cigarElement.getOperator() == CigarOperator.D) {
              // if (pos > 0)
              mask[pos] = true;
              if (pos + 1 < mask.length) mask[pos + 1] = true;
            }
          ...
          for (final CigarElement cigarElement : samRecord.getCigar().getCigarElements()) {
            if (cigarElement.getOperator().consumesReadBases()) {
              for (int i = 0; i < cigarElement.getLength(); i++) {
                final boolean match =
                    samRecord.getReadBases()[pos + i] == referenceTracks.baseAt(refPos + i);
                mask[pos + i] =
                    (baseCategory.type == BaseCategoryType.MATCH && match)
        ...
        case INSERTION:
          ...
          for (final CigarElement cigarElement : samRecord.getCigar().getCigarElements()) {
            switch (cigarElement.getOperator()) {
              case I:
                for (int i = 0; i < cigarElement.getLength(); i++) mask[pos + i] = true;
                break;
              default:
                break;
            }
        ...
        case PILEUP:
          for (int i = 0; i < qualityScores.length; i++)
            if (referenceTracks.mismatchesAt(alStart + i) > baseCategory.param) mask[i] = true;
          break;
    ...
    // to store:
    if (maskedCount > samRecord.getReadLength() / 2) cramRecord.setForcePreserveQualityScores(true);
  }
}
