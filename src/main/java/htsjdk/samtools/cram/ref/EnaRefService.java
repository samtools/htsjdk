...
public class EnaRefService {
  ...
  /**
   * Tries to download sequence bases using its md5 checksum. This method can try downloading the
   * sequence many times before giving up.
   *
   * @param md5 MD5 checksum string of the sequence to download
   * @param timeoutMs timeout in milliseconds before failing with the {@link
   *     EnaRefService.GaveUpException}
   * @param maxTries maximum number of tries before failing with the {@link
   *     EnaRefService.GaveUpException}
   ...
   * @throws GaveUpException if the sequence could not be downloaded within the time/try limit.
   */
  byte[] getSequence(
      final String md5, final long timeoutMs, int maxTries, final long restBetweenTriesMs)
      ...
      try {
        ...
        switch (code) {
          ...
          case HTTP_MOVED_PERMANENTLY:
            log.error(
                "It seems that the base URL for the ENA service has changed permanently. Got error:"
                    + code
