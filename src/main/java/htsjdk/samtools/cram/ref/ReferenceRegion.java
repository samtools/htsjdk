...
class ReferenceRegion {
  ...
  public ReferenceRegion(
    ...
    if (alignmentEnd == -1) alignmentEnd = bases.length;
    ...
    if (alignmentStart < 1
      ...
      throw new IllegalArgumentException(
          String.format(
              "Invalid reference region: %s, %d, %d.", sequenceName, alignmentStart, alignmentEnd));
  ...
  int arrayPosition(final long alignmentPosition) {
    ...
    if (arrayPosition < 0 || arrayPosition > array.length)
      throw new IllegalArgumentException(
          "The alignment position is out of the region: " + alignmentPosition);
