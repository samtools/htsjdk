...
/**
 * Used to represent a CRAM reference, the backing source for which can either be a file or the EBI
 * ENA reference service.
 *
 * <p>NOTE: In a future release, this class will be renamed and the functionality it contains will
 * be refactored and distributed into one or more separate reference source implementations, each
 * corresponding to the type of resource backing the reference.
 */
public class ReferenceSource implements CRAMReferenceSource {
  ...
  /**
   * Attempts to construct a default CRAMReferenceSource for use with CRAM files when one has not
   * been explicitly provided.
   *
   * @return CRAMReferenceSource if one can be acquired. Guaranteed to not be null if none of the
   *     listed exceptions is thrown.
   * @throws IllegalStateException if no default reference source can be acquired
   * @throws IllegalArgumentException if the reference_fasta environment variable refers to a a file
   *     that doesn't exist
   *     <p>Construct a default reference source to use when an explicit reference has not been
   *     provided by checking for fallback sources in this order:
   *     <p>
   *     <ul>
   *       <li>Defaults.REFERENCE_FASTA - the value of the system property "reference_fasta". If
   *           set, must refer to a valid reference file.
   *       <li>ENA Reference Service if it is enabled
   *     </ul>
   */
    ...
    if (null != Defaults.REFERENCE_FASTA) {
      if (Defaults.REFERENCE_FASTA.exists()) {
        ...
        throw new IllegalArgumentException(
            "The file specified by the reference_fasta property does not exist: "
  ...
  private byte[] addToCache(final String sequenceName, final byte[] bases) {
    // Normalize to upper case only. We can't use the cram normalization utility
    // Utils.normalizeBases, since
    // we don't want to normalize ambiguity codes, we can't use SamUtils.normalizeBases, since we
    // don't want
  ...
  @Override
  public synchronized byte[] getReferenceBases(
      final SAMSequenceRecord record, final boolean tryNameVariants) {
  ...
  byte[] findBasesByMD5(final String md5) throws IOException {
