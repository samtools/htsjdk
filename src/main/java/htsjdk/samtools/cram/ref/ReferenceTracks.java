...
public class ReferenceTracks {
  ...
  public ReferenceTracks(final int sequenceId, final String sequenceName, final byte[] reference) {
    this(sequenceId, sequenceName, reference, DEFAULT_WINDOW_SIZE);
  }

  private ReferenceTracks(
  ...
  public void ensure(final int start, final int end) {
    ...
    if (position < start) moveForwardTo(start);
  }

  /**
   * Shift the window forward to a new position on the reference.
   *
   * @param newPos 1-based reference coordinate, must be greater than current position and smaller
  ...
  void moveForwardTo(int newPos) {
    ...
    if (newPos > reference.length - bases.length + 1) newPos = reference.length - bases.length + 1;

    if (newPos == position) return;

    System.arraycopy(
        reference, newPos - 1, bases, 0, Math.min(bases.length, reference.length - newPos + 1));
  ...
  void reset() {
    System.arraycopy(
        reference, position - 1, bases, 0, Math.min(bases.length, reference.length - position + 1));
    Arrays.fill(coverage, (short) 0);
    Arrays.fill(mismatches, (short) 0);
  }

  public void ensureRange(final int start, final int length) {
    if (start < position) throw new RuntimeException("Cannot move the window backwards: " + start);

    if (start > position || start + length > position + bases.length) moveForwardTo(start);
  }

  public final byte baseAt(final int pos) {
    if (pos - this.position < coverage.length) return bases[pos - this.position];
    else return 'N';
  }

  public final short coverageAt(final int pos) {
    if (pos - this.position >= coverage.length) return 0;
    else {
  ...
  public final short mismatchesAt(final int pos) {
    if (pos - this.position >= coverage.length) return 0;
    else return mismatches[pos - this.position];
  }

  public final void addCoverage(final int pos, final int amount) {
    if (pos - this.position < coverage.length) coverage[pos - this.position] += amount;
  }

  public final void addMismatches(final int pos, final int amount) {
    if (pos - this.position < coverage.length) mismatches[pos - this.position] += amount;
  }
}
