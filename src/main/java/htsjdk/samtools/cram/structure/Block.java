...
/**
 * Class representing CRAM block concept and some methods to operate with block content. CRAM block
 * is used to hold some (usually homogeneous) binary data. An external compression can be applied to
 * the content of a block. The class provides some instantiation static methods, for example to read
 * a block from an input stream. Blocks can be written out to an output stream, this may be
 * considered as a way to serialize/deserialize blocks.
 */
public class Block {
  ...
  /**
   * Deserialize the block from the {@link InputStream}. The reading is parametrized by the major
   * CRAM version number.
   *
   ...
   */
  public static Block readFromInputStream(final int major, InputStream inputStream)
      throws IOException {
    ...
    InputStreamUtils.readFully(
        inputStream, block.compressedContent, 0, block.compressedContent.length);
    if (v3OrHigher) {
      final int actualChecksum = ((CRC32InputStream) inputStream).getCRC32();
      final int checksum = CramInt.int32(inputStream);
      if (checksum != actualChecksum)
        throw new RuntimeException(
            String.format("Block CRC32 mismatch: %04x vs %04x", checksum, actualChecksum));
    }
  ...
  /**
   * Create a new slice header block with the given uncompressed content. The block wil have RAW (no
   * compression) and MAPPED_SLICE content type.
   *
  ...
  /**
   * Create a new core block with the given uncompressed content. The block wil have RAW (no
   * compression) and CORE content type.
   *
   * @param rawContent the content of the block
  ...
  public String toString() {
    final String raw =
        rawContent == null
            ? "NULL"
            : Arrays.toString(Arrays.copyOf(rawContent, Math.min(5, rawContent.length)));
    final String comp =
        compressedContent == null
            ? "NULL"
            : Arrays.toString(
                Arrays.copyOf(compressedContent, Math.min(5, compressedContent.length)));

    return String.format(
        "method=%s, type=%s, id=%d, raw size=%d, compressed size=%d, raw=%s, comp=%s.",
        getMethod().name(),
        getContentType().name(),
        getContentId(),
        rawContentSize,
  ...
  /**
   * Write the block out to the the specified {@link OutputStream}. The method is parametrized with
   * CRAM major version number.
   *
   * @param major CRAM version major number
