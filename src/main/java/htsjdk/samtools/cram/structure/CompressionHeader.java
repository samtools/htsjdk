...
public class CompressionHeader {
  ...
  public final Map<Integer, ExternalCompressor> externalCompressors =
      new HashMap<Integer, ExternalCompressor>();
  ...
  private byte[][][] parseDictionary(final byte[] bytes) {
    ...
    int maxWidth = 0;
    for (final List<byte[]> list : dictionary) maxWidth = Math.max(maxWidth, list.size());
  ...
  void read(final InputStream is) throws IOException {
    { // preservation map:
      ...
      for (int i = 0; i < mapSize; i++) {
        ...
        if (RN_readNamesIncluded.equals(key)) readNamesIncluded = buffer.get() == 1;
        ...
        } else throw new RuntimeException("Unknown preservation map key: " + key);
      }
    }
    ...
    { // encoding map:
      ...
      for (int i = 0; i < mapSize; i++) {
        ...
        log.debug(
            String.format(
                "FOUND ENCODING: %s, %s, %s.",
                encodingKey.name(), id.name(), Arrays.toString(Arrays.copyOf(paramBytes, 20))));
      }
    }
  ...
  void write(final OutputStream outputStream) throws IOException {
    ...
    { // encoding map:
      ...
      for (final EncodingKey encodingKey : encodingMap.keySet()) {
        if (encodingMap.get(encodingKey).id != EncodingID.NULL) size++;
      }

      final ByteBuffer mapBuffer = ByteBuffer.allocate(1024 * 100);
      ITF8.writeUnsignedITF8(size, mapBuffer);
      for (final EncodingKey encodingKey : encodingMap.keySet()) {
        if (encodingMap.get(encodingKey).id == EncodingID.NULL) continue;
