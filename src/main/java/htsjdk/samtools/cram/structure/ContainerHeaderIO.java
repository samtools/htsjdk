...
class ContainerHeaderIO {
  ...
  public boolean readContainerHeader(
      final int major, final Container container, final InputStream inputStream)
      throws IOException {
    ...
    for (int i = 1; i < peek.length; i++) {
      ...
      if (character == -1) throw new RuntimeException("Incomplete or broken stream.");
    ...
    if (major >= 3) container.checksum = CramInt.int32(inputStream);
   ...
   */
  public int writeContainerHeader(
