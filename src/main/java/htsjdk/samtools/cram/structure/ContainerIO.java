...
public class ContainerIO {
  ...
  /**
   * Reads a CRAM container from the input stream. Returns an EOF container when there is no more
   * data or the EOF marker found.
   *
   ...
   * @return a new {@link Container} object with container header values filled out but empty body
   *     (no slices and blocks).
   * @throws IOException as per java IO contract
   */
  public static Container readContainerHeader(final int major, final InputStream inputStream)
    ...
    if (!containerHeaderIO.readContainerHeader(major, container, inputStream)) {
      containerHeaderIO.readContainerHeader(
          container,
          new ByteArrayInputStream(
              (major >= 3 ? CramIO.ZERO_F_EOF_MARKER : CramIO.ZERO_B_EOF_MARKER)));
      return container;
    }
    return container;
  }
  ...
  private static Container readContainer(
      final int major, final InputStream inputStream, final int fromSlice, int howManySlices)
      throws IOException {
   ...
   * @throws IOException as per java IO contract
   */
  public static int writeContainerHeader(
      final int major, final Container container, final OutputStream outputStream)
      throws IOException {
    return new ContainerHeaderIO().writeContainerHeader(major, container, outputStream);
  }

  /**
   * Writes a complete {@link Container} with it's header to a {@link OutputStream}. The method is
   * aware of file header containers and is suitable for general purpose use: basically any
   ...
   * @throws IOException as per java IO contract
   */
  public static int writeContainer(
      final Version version, final Container container, final OutputStream outputStream)
      throws IOException {
    {
      if (container.blocks != null && container.blocks.length > 0) {

        final Block firstBlock = container.blocks[0];
        final boolean isFileHeaderContainer =
            firstBlock.getContentType() == BlockContentType.FILE_HEADER;
        if (isFileHeaderContainer) {
          final ExposedByteArrayOutputStream byteArrayOutputStream =
          ...
          final int containerHeaderByteSize =
              new ContainerHeaderIO().writeContainerHeader(version.major, container, outputStream);
    ...
    for (int i = 0; i < container.landmarks.length; i++) container.landmarks[i] = landmarks.get(i);
  ...
  public static long getByteSize(final Version version, final Container container) {
    final CountingOutputStream countingOutputStream =
