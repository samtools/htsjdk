...
/**
 * A starting object when dealing with CRAM files. A {@link CramHeader} holds 2 things: 1. File
 * format definition, including content id and version information 2. SAM file header
 */
public final class CramHeader {
  ...
  /**
   * Create a new {@link CramHeader} object with the specified version, id and SAM file header. The
  ...
  /**
   * Set the id of the header. A typical use is for example file name to be used when streaming or a
   * checksum of the data contained in the file.
   *
   * @param stringID a new id; only first 20 bytes from byte representation of java {@link String}
   *     will be used.
   */
  public void setID(final String stringID) {
    System.arraycopy(
        stringID.getBytes(), 0, this.id, 0, Math.min(this.id.length, stringID.length()));
  }
