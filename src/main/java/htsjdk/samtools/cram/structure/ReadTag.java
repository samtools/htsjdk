...
/**
 * CRAM counterpart of {@link htsjdk.samtools.SAMTag}. TODO: consider merging/dropping this class in
 * favour of SAMTag or a SAMTag implementation.
 */
public class ReadTag implements Comparable<ReadTag> {
  ...
  public ReadTag(
      final int id, final byte[] dataAsByteArray, ValidationStringency validationStringency) {
  ...
  private ReadTag(final String key, final char type, final Object value) {
    if (key == null) throw new NullPointerException("Tag key cannot be null.");
    if (value == null) throw new NullPointerException("Tag value cannot be null.");
  ...
  public static ReadTag deriveTypeFromValue(final String key, final Object value) {
    if (key.length() != 2) throw new RuntimeException("Tag key must be 2 char long: " + key);
  ...
  public boolean equals(final Object obj) {
    if (!(obj instanceof ReadTag)) return false;

    final ReadTag foe = (ReadTag) obj;
    return key.equals(foe.key)
        && (value == null && foe.value == null || value != null && value.equals(foe.value));
  }
  ...
  private static char getTagValueType(final Object value) {
    if (value instanceof String) {
      ...
      throw new IllegalArgumentException(
          "When writing BAM, unrecognized tag type " + value.getClass().getName());
    }
  }
  ...
  private static final ThreadLocal<ByteBuffer> bufferLocal =
  ...
  public static byte[] writeSingleValue(
      final byte tagType, final Object value, final boolean isUnsignedArray) {
  ...
  private static void writeArray(
      final Object value, final boolean isUnsignedArray, final ByteBuffer buffer) {
    if (value instanceof byte[]) {
      buffer.put((byte) (isUnsignedArray ? 'C' : 'c'));
      final byte[] array = (byte[]) value;
      buffer.putInt(array.length);
      for (final byte element : array) buffer.put(element);

    } else if (value instanceof short[]) {
      buffer.put((byte) (isUnsignedArray ? 'S' : 's'));
      final short[] array = (short[]) value;
      buffer.putInt(array.length);
      for (final short element : array) buffer.putShort(element);

    } else if (value instanceof int[]) {
      buffer.put((byte) (isUnsignedArray ? 'I' : 'i'));
      final int[] array = (int[]) value;
      buffer.putInt(array.length);
      for (final int element : array) buffer.putInt(element);

    } else if (value instanceof float[]) {
      buffer.put((byte) 'f');
      final float[] array = (float[]) value;
      buffer.putInt(array.length);
      for (final float element : array) buffer.putFloat(element);

    } else throw new SAMException("Unrecognized array value type: " + value.getClass());
  }

  public static Object readSingleValue(
      final byte tagType, final ByteBuffer byteBuffer, ValidationStringency validationStringency) {
    switch (tagType) {
      case 'Z':
      ...
      case 'I':
        ...
        // If it won't fit into a signed integer, but is within range for an unsigned 32-bit
        // integer,
      ...
      case 'B':
        final TagValueAndUnsignedArrayFlag valueAndFlag = readArray(byteBuffer);
   ...
   * @return CVO containing the value in in-memory Object form, and a flag indicating whether it is
   *     unsigned or not.
   */
