 ...
 * except in compliance with the License. You may obtain a copy of the License at4
 *
...
public class Slice {
  ...
  private void alignmentBordersSanityCheck(final byte[] ref) {
    ...
    if (alignmentStart > ref.length) {
      log.error(
          String.format(
              "Slice mapped outside of reference: seqID=%d, start=%d, counter=%d.",
    ...
    if (alignmentStart - 1 + alignmentSpan > ref.length) {
      log.warn(
          String.format(
              "Slice partially mapped outside of reference: seqID=%d, start=%d, span=%d, counter=%d.",
  ...
  public boolean validateRefMD5(final byte[] ref) {
    if (sequenceId == Slice.MULTI_REFERENCE)
      throw new SAMException(
          "Cannot verify a slice with multiple references on a single reference.");
    ...
    if (!validateRefMD5(ref, alignmentStart, alignmentSpan, refMD5)) {
      ...
      if (validateRefMD5(ref, alignmentStart, alignmentSpan - 1, refMD5)) {
        log.warn(
            String.format(
                "Reference MD5 matches partially for slice %d:%d-%d, %s",
                sequenceId, alignmentStart, alignmentStart + alignmentSpan - 1, excerpt));
        return true;
      }

      log.error(
          String.format(
              "Reference MD5 mismatch for slice %d:%d-%d, %s",
              sequenceId, alignmentStart, alignmentStart + alignmentSpan - 1, excerpt));
      return false;
    }
  ...
  private static boolean validateRefMD5(
      final byte[] ref,
  ...
  private static String getBrief(
      final int startOneBased, final int span, final byte[] bases, final int shoulderLength) {
    if (span >= bases.length) return new String(bases);
  ...
  public String toString() {
    return String.format(
        "slice: seqID %d, start %d, span %d, records %d.",
  ...
  public void setRefMD5(final byte[] ref) {
    ...
    if (sequenceId < 0 && alignmentStart < 1) {
      ...
      if (log.isEnabled(Log.LogLevel.DEBUG)) {
        ...
        if (ref.length <= shoulder * 2) sb.append(new String(ref));
        else {
        ...
        log.debug(
            String.format(
                "Slice md5: %s for %d:%d-%d, %s",
                String.format("%032x", new BigInteger(1, refMD5)),
                sequenceId,
                alignmentStart,
                alignmentStart + span - 1,
                sb.toString()));
      }
    }
  }
   ...
   *
   * @param tag tag ID as a short integer as returned by {@link
   *     htsjdk.samtools.SAMTagUtil#makeBinaryTag(java.lang.String)}
