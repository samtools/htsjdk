...
class SliceIO {
  ...
  private static void readSliceHeadBlock(
      final int major, final Slice slice, final InputStream inputStream) throws IOException {
  ...
  private static void parseSliceHeaderBlock(final int major, final Slice slice) throws IOException {
    ...
    if (major >= CramVersions.CRAM_v3.major) {
      slice.sliceTags =
          BinaryTagCodec.readTags(bytes, 0, bytes.length, ValidationStringency.DEFAULT_STRINGENCY);

      SAMBinaryTagAndValue tags = slice.sliceTags;
      while (tags != null) {
        log.debug(
            String.format(
                "Found slice tag: %s", SAMTagUtil.getSingleton().makeStringTag(tags.tag)));
        tags = tags.getNext();
      }
    }
  }
  ...
  private static byte[] createSliceHeaderBlockContent(final int major, final Slice slice)
    ...
    int i = 0;
    for (final int id : slice.external.keySet()) slice.contentIDs[i++] = id;
    ...
    if (major >= CramVersions.CRAM_v3.major) {
      if (slice.sliceTags != null) {
        ...
        do {
          log.debug(
              "Writing slice tag: "
                  + SAMTagUtil.getSingleton().makeStringTag(samBinaryTagAndValue.tag));
  ...
  private static void readSliceBlocks(
      final int major, final Slice slice, final InputStream inputStream) throws IOException {
      ...
      switch (block.getContentType()) {
        ...
        default:
          throw new RuntimeException(
              "Not a slice block, content type id " + block.getContentType().name());
      }
    }
  }
  ...
  public static void write(final int major, final Slice slice, final OutputStream outputStream)
    ...
    for (final Block block : slice.external.values()) block.write(major, outputStream);
  }
