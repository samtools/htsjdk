...
public class SubstitutionMatrix {
  ...
  public SubstitutionMatrix(final byte[] matrix) {
    ...
    for (final byte refBase : BASES) {
      for (byte code = 0; code < 4; code++) codes[refBase][bases[refBase][code]] = code;
    }
  }
  ...
  private static final Comparator<SubCode> comparator =
      new Comparator<SubstitutionMatrix.SubCode>() {
  ...
  private byte rank(final byte refBase, final long[] frequencies) {
    ...
    for (byte i = 0; i < subCodes.length; i++) subCodes[i].rank = i;
    ...
    for (final SubCode s : subCodes) codes[refBase][s.base] = s.rank;
