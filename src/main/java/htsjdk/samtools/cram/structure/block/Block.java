...
public class Block {
    ...
    public final byte[] getUncompressedContent(final CompressorCache compressorCache) {
        ...
        if (uncompressedContent.length != uncompressedLength) {
            //TODO: remove this - temporary check to account for the (temporarily) missing terminator in the htsjdk implementation
            if (compressionMethod.equals(BlockCompressionMethod.NAME_TOKENISER)) {
                if (uncompressedContent.length != uncompressedLength - 1) {
