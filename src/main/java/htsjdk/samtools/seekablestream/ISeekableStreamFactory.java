...
/**
 * Factory for creating {@link SeekableStream}s based on URLs/paths. Implementations can be set as
 * the default with {@link SeekableStreamFactory#setInstance(ISeekableStreamFactory)}
 *
 * @author jacob
 * @date 2013-Oct-24
 */
public interface ISeekableStreamFactory {
  ...
  /**
   * Return a buffered {@code SeekableStream} which wraps the input {@code stream} using the default
   * buffer size
   *
   * @param stream
   * @return
   ...
   *
   * <p>The wrapper allows applying operations directly to the byte stream so that things like
   * caching, prefetching, or decryption can be done at the raw byte level.
   *
   * <p>The default implementation throws if wrapper != null, but implementations may support this
   * wrapping operation
   *
   ...
   */
  default SeekableStream getStreamFor(
      String path, Function<SeekableByteChannel, SeekableByteChannel> wrapper) throws IOException {
    if (wrapper != null) {
