...
/**
 * Describes a single SRA accession for SRA read collection Also provides app string functionality
 * and allows to check if working SRA is supported on the running platform
 *
 * <p>Important: due to checks performed in SRAAccession.isValid(), we won't recognise any
 * accessions other than ones that follow the pattern "^[SED]RR[0-9]{6,9}$", e.g. SRR000123
 */
public class SRAAccession implements Serializable {
  ...
  /**
   * @return true if SRA successfully loaded native libraries and fully initialized, false otherwise
  ...
  /**
   * Tries to initialize SRA. Initialization error is saved during first call, all subsequent calls
   * will return the same saved error or null.
   *
   * @return ExceptionInInitializerError if initialization failed, null if initialization was
   *     successful
   */
