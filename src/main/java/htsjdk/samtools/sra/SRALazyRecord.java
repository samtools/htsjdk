...
/**
 * Extends SAMRecord so that any of the fields will be loaded only when needed. Since SRA is a
 * column oriented database, it is very inefficient to load all the fields at once. However, loading
 * only set of actually needed fields will be even faster than in row oriented databases.
 *
 * <p>Because of that we are providing lazy loading of fields, flags and attributes.
 *
 * <p>Created by andrii.nikitiuk on 8/25/15.
 */
public class SRALazyRecord extends SAMRecord {
  ...
  protected SRALazyRecord(
      final SAMFileHeader header, SRAAccession accession, String readId, String alignmentId) {
  ...
  private SRALazyRecord(
      final SAMFileHeader header, SRAAccession accession, String readId, boolean isAligned) {
    super(header);
  ...
  /**
   * Is being called when original NGS iterator is being moved to the next object. Later, if any of
   * uninitialized fields is requested, either Read object or Alignment has to be retrieved from
   * ReadCollection
   */
   ...
   * Since read id is a constructor parameter, we only need to make sure that reference info is
   * loaded.
   *
   * @param o other
  ...
  private Alignment getCurrentAlignment() throws ErrorMsg {
    ...
    if (alignmentIterator == null) {
      log.debug(
          "Recovering SAM record after detaching from iterator. Alignment id: " + sraAlignmentId);
      if (sraAlignmentId == null) {
        throw new RuntimeException(
            "Cannot recover SAM object after detaching from iterator: no alignment id");
      }
  ...
  private Read getCurrentUnalignedRead() throws ErrorMsg {
    ...
    if (unalignmentIterator == null) {
      log.debug(
          "Recovering SAM record after detaching from iterator. Read id: "
              + sraReadId
              + ", fragment index: "
              + unalignedReadFragmentIndex);
      if (sraReadId == null) {
        throw new RuntimeException(
            "Cannot recover SAM object after detaching from iterator: no read id");
      }
  ...
  private byte[] getBaseQualitiesImpl() {
    try {
      ...
      // quals are being taken from PRIMARY_ALIGNMENT.SAM_QUALITY column which reverse automatically
      // them if needed
