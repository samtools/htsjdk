...
public class SRAUtils {
  /**
   * References are stored in SRA table in chunks of 5k bases per row, while last chunk of a
   * reference is less or equal than 5k bases in size (even if the next reference follows). So, it
   * will be optimal if we align reference sizes to 5k bases to read by reference rows.
   */
  ...
  /**
   * Loads reference lengths from a read collection. Aligns reference lengths by REFERENCE_ALIGNMENT
   * bases for optimal loads of alignments (references are stored in REFERENCE_ALIGNMENT bases
   * chunks in SRA table)
   *
   * <p>Is used to build RecordRangeInfo
   *
