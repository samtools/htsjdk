...
/**
 * Abstract class that is designed to be extended and specialized to provide an asynchronous wrapper
 * around any kind of Writer class that takes an object and writes it out somehow.
 *
 * <p>NOTE: Objects of subclasses of this class are not intended to be shared between threads. In
 * particular there must be only one thread that calls {@link #write} and {@link #close}.
 *
 * <p>NOTE: Any exception thrown by the underlying Writer will be propagated back to the caller
 * during the next available call to {@link #write} or {@link #close}. After the exception has been
 * thrown to the caller, it is not safe to attempt further operations on the instance.
 *
 * @author Tim Fennell
 */
public abstract class AbstractAsyncWriter<T> implements Closeable {
  ...
  /**
   * Public method for sub-classes or ultimately consumers to put an item into the queue to be
   * written out.
   */
  ...
  /**
   * Attempts to finish draining the queue and then calls synchronouslyClose() to allow
   * implementation to do any one time clean up.
   */
  @Override
  public void close() {
    checkAndRethrow();
    ...
    if (!this.isClosed.getAndSet(true)) {
      ...
      // The queue should be empty but if it's not, we'll drain it here to protect against any lost
      // data.
      // There's no need to timeout on poll because poll is called only when queue is not empty and
      // at this point the writer thread is definitely dead and noone is removing items from the
      // queue.
  ...
  /**
   * Checks to see if an exception has been raised in the writer thread and if so rethrows it as an
   * Error or RuntimeException as appropriate.
   */
  ...
  private class WriterRunnable implements Runnable {
    ...
    public void run() {
      try {
        // The order of the two conditions is important, see
        // https://github.com/samtools/htsjdk/issues/564
        // because we want to make sure that emptiness status of the queue does not change after we
        // have evaluated isClosed
