 ...
 * jrobinso
 *
 * <p>the feature reader class, which uses indices and codecs to read in Tribble file formats.
 */
public abstract class AbstractFeatureReader<T extends Feature, SOURCE> implements FeatureReader<T> {
  ...
  // a wrapper to apply to the raw stream of the Feature file to allow features like prefetching and
  // caching to be injected
   ...
   */
  public static <FEATURE extends Feature, SOURCE>
      AbstractFeatureReader<FEATURE, SOURCE> getFeatureReader(
  ...
  /**
   * {@link #getFeatureReader(String, String, FeatureCodec, boolean, Function, Function)} with
   * {@code null} for indexResource, wrapper, and indexWrapper
   *
   * @throws TribbleException
   */
  public static <FEATURE extends Feature, SOURCE>
  ...
  /**
   * {@link #getFeatureReader(String, String, FeatureCodec, boolean, Function, Function)} with
   * {@code null} for wrapper, and indexWrapper
   *
   * @throws TribbleException
   */
  public static <FEATURE extends Feature, SOURCE>
   ...
   * @param wrapper a wrapper to apply to the byte stream from the featureResource allowing
   *     injecting features like caching and prefetching of the stream, may be null, will only be
   *     applied if featureResource is a uri representing a {@link java.nio.file.Path}
   * @param indexWrapper a wrapper to apply to the byte stream from the indexResource, may be null,
   *     will only be applied if indexResource is a uri representing a {@link java.nio.file.Path}
   * @throws TribbleException
   */
  public static <FEATURE extends Feature, SOURCE>
      AbstractFeatureReader<FEATURE, SOURCE> getFeatureReader(
    ...
    try {
      ...
      if (methods.isTabix(featureResource, indexResource)) {
        ...
        return new TabixFeatureReader<>(
            featureResource, indexResource, (AsciiFeatureCodec) codec, wrapper, indexWrapper);
      }
      // Not tabix => tribble index file (might be gzipped, but not block gzipped)
      else {
        return new TribbleIndexedFeatureReader<>(
            featureResource, indexResource, codec, requireIndex, wrapper, indexWrapper);
      }
  ...
  protected AbstractFeatureReader(
      final String path,
