...
public interface FeatureReader<T extends Feature> extends Closeable {
   ...
   * @throws IOException If there's a problem reading or if the reader is not queryable, e.g. if it
   *     doesn't have an index.
   */
  CloseableTribbleIterator<T> query(final String chr, final int start, final int end)
      throws IOException;
  ...
  /**
   * @return true if the reader has an index, which means that it can be queried. Must be
   *     implemented to return true if the implementation supports {@link #query(Locatable)}
   */
