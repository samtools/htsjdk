 ...
 * Discontinuous features which are split between multiple lines in the gff files are implemented as
 * separate features linked as "co-features"
 */
public class Gff3FeatureImpl implements Gff3Feature {
    ...
    /**
     * top level features are features with no parents. Each feature maintains a list of its top
     * level features, from which it and all related features descend.
     */
    ...
    public Gff3FeatureImpl(final String contig, final String source, final String type,
        ...
        baseData = new Gff3BaseData(contig, source, type, start, end, score, strand, phase,
                attributes);
     ...
     * Derives_From can be used to specify a particular inheritance path for this feature when
    ...
    public Set<Gff3FeatureImpl> getAncestors() {
        ...
        for (final Gff3FeatureImpl parent : parents) {
            ancestors.addAll(
                    getAttribute(DERIVES_FROM_ATTRIBUTE_KEY).isEmpty() ? parent.getAncestors()
                            : parent.getAncestors(new HashSet<>(
                                    baseData.getAttributes().get(DERIVES_FROM_ATTRIBUTE_KEY))));
        }
    ...
    private Set<Gff3FeatureImpl> getDescendents(final Set<String> idsInLineage) {
        final List<Gff3FeatureImpl> childrenToAdd = children.stream()
                .filter(c -> c.getAttribute(DERIVES_FROM_ATTRIBUTE_KEY).isEmpty() || !Collections
                        .disjoint(idsInLineage, c.getAttribute(DERIVES_FROM_ATTRIBUTE_KEY)))
        ...
        updatedIdsInLineage
                .addAll(childrenToAdd.stream().map(Gff3Feature::getID).collect(Collectors.toSet()));
    ...
    /**
     * Get set of co-features. Co-features correspond to the other lines in the gff file that
     * together make up a single discontinuous feature
     * 
    ...
    public void addParent(final Gff3FeatureImpl parent) {
        final Set<Gff3FeatureImpl> topLevelFeaturesToAdd =
                new HashSet<>(parent.getTopLevelFeatures());
        if (!getAttribute(DERIVES_FROM_ATTRIBUTE_KEY).isEmpty()) {
            topLevelFeaturesToAdd.removeIf(f -> !getAttribute(DERIVES_FROM_ATTRIBUTE_KEY)
                    .contains(f.getID())
                    && f.getDescendents().stream().noneMatch(f2 -> f2.getID() != null
                            && getAttribute(DERIVES_FROM_ATTRIBUTE_KEY).contains(f2.getID())));
        }
        parents.add(parent);
    ...
    /**
     * Add a feature as a coFeature of this feature. When this method is called, the input coFeature
     * will also be added as a coFeature of all the other coFeatures of this object, and this
     * feature and all coFeatures will be added as coFeatures of the input coFeature. All coFeatures
     * must have equal IDs and parents.
     * 
    ...
    public void addCoFeature(final Gff3FeatureImpl coFeature) {
        if (!parents.equals(coFeature.getParents())) {
            ...
            throw new TribbleException(
                    "Co-features " + baseData.getId() + " do not have same parents");
        }
    ...
    public boolean equals(Object other) {
        ...
        /*
         * to test for equality, the doubly linked list representation used to represent feature
         * relationships is replaced with a graph representation. equality for between two features
         * is tested by testing equality between their base data fields, and equality between the
         * graphs they are part of.
         */
    ...
    public Set<Gff3FeatureImpl> flatten() {
        final LinkedHashSet<Gff3FeatureImpl> features =
                new LinkedHashSet<>(Collections.singleton(this));
    ...
    /**
     * Class for graph representation of relationships between features. Used for testing equality
     * between features
     */
    private static class Gff3Graph {
        ...
        Gff3Graph(final Gff3Feature feature) {
            feature.getTopLevelFeatures().stream().flatMap(f -> f.flatten().stream())
        ...
        private void addCoFeatureSet(final Gff3Feature feature) {
            if (feature.hasCoFeatures()) {
                final Set<Gff3BaseData> coFeaturesBaseData = feature.getCoFeatures().stream()
                        .map(Gff3Feature::getBaseData).collect(Collectors.toSet());
    ...
    public String toString() {
        return "Gff3FeatureImpl [baseData=" + baseData + " , parents:" + this.parents.size()
