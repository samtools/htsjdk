...
/**
 * A class to write out gff3 files. Features are added using {@link #addFeature(Gff3Feature)},
...
public class Gff3Writer extends AbstractGxxWriter {
    ...
    protected void writeAttributes(final Map<String, List<String>> attributes) throws IOException {
        ...
        writeJoinedByDelimiter(Gff3Constants.ATTRIBUTE_DELIMITER,
                e -> writeKeyValuePair(e.getKey(), e.getValue()), attributes.entrySet());
    }
    ...
    void writeKeyValuePair(final String key, final List<String> values) {
        try {
            ...
            writeJoinedByDelimiter(Gff3Constants.VALUE_DELIMITER, v -> tryToWrite(escapeString(v)),
                    values);
        } catch (final IOException ex) {
    ...
    public void addDirective(final Gff3Codec.Gff3Directive directive, final Object object)
        ...
        if (directive == Gff3Codec.Gff3Directive.VERSION3_DIRECTIVE) {
            throw new TribbleException(
                    "VERSION3_DIRECTIVE is automatically added and should not be added manually.");
        }
