...
public class IntervalIndexCreator extends TribbleIndexCreator {
  ...
  public void addFeature(final Feature feature, final long filePosition) {
    // if we don't have a chrIndex yet, or if the last one was for the previous contig, create a new
    // one
    if (chrList.isEmpty() || !chrList.getLast().getName().equals(feature.getContig())) {
      // if we're creating a new chrIndex (not the first), make sure to dump the intervals to the
      // old chrIndex
    ...
    intervals
        ...
        .setStop(Math.max(feature.getEnd(), intervals.get(intervals.size() - 1).getStop()));
    featureCount++;
  }
...
/**
 * The interval class isn't mutable; use this private class as a temporary storage until we're ready
 * to make intervals
 */
class MutableInterval {
  ...
  public Interval toInterval() {
    return new Interval(
        start, stop, new Block(startFilePosition, endFilePosition - startFilePosition));
  }
