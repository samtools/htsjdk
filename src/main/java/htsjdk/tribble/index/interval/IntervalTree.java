 ...
 *
 * <p>References: http://en.wikipedia.org/wiki/Interval_tree
 *
 * <p>Cormen, Thomas H.; Leiserson, Charles E., Rivest, Ronald L. (1990). Introduction to Algorithms
 * (1st ed.). MIT Press and McGraw-Hill. ISBN 0-262-03141-8
 */
public class IntervalTree {
  ...
  /**
   * The estimated size of the tree. We keep a running count on each insert, this getter returns
   * that count.
   *
   * @return
   * @see #size()
  ...
  /**
   * Get all nodes which are descendants of {@code node}, inclusive. {@code results} is modified in
   * place
   *
   * @param node
   * @param results
  ...
  /**
   * @return Returns the number of nodes in the tree. Recalculated each call
   * @see #getSize()
   */
  public int size() {
  ...
  /**
   * Test code: make sure that the tree has all the properties defined by Red Black trees and
   * interval trees
   *
   * <p>o. Root is black.
   *
   * <p>o. NIL is black.
   *
   * <p>o. Red nodes have black children.
   *
   * <p>o. Every path from root to leaves contains the same number of black nodes.
   *
   * <p>o. getMax(node) is the maximum of any interval rooted at that node..
   *
   * <p>This code is expensive, and only meant to be used for assertions and testing.
   */
  ...
  static class Node {
    ...
    // Color and parent are used for inserts.  If tree is immutable these are not required (no
    // requirement
