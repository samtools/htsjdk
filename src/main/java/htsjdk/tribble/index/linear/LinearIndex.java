...
/**
 * Index defined by dividing the genome by chromosome, then each chromosome into bins of fixed width
 * (in genomic coordinates). Features are allocated to bins by start position. The longest feature
 * in each recorded and used to adjust the start position of a query to include all bins that might
 * have a feature that overlaps the query interval. This works well for feature sets of
 * approximately homogeneous length, or whose longest feature is on the order of the bin width or
 * less.
 *
 * <p>magicNumber integer type integer version integer filename null terminated character array
 * filesize long lastModified long md5 String flags integer
 *
 * <p>------ LINEAR INDEX nChromosomes integer
 */
public class LinearIndex extends AbstractIndex {
  ...
  public static final double MAX_FEATURES_PER_BIN =
      Double.valueOf(System.getProperty("MAX_FEATURES_PER_BIN", "100"));
  ...
  // 1MB: we will no merge bins with any features in them beyond this size, no matter how sparse,
  // per chromosome
  private static final long MAX_BIN_WIDTH_FOR_OCCUPIED_CHR_INDEX =
      Long.valueOf(System.getProperty("MAX_BIN_WIDTH_FOR_OCCUPIED_CHR_INDEX", "1024000"));
  ...
  public boolean isCurrentVersion() {
    ...
    // todo fixme nasty hack to determine if this is an old style V3 linear index (without
    // nFeaturesPerBin)
   ...
   *
   * <p>Block 0 Block 1 Block 2
   *
   * <p>There's a constant bin width, so that each block corresponds to a specific interval over the
   * genome based on its index, as in:
   *
   * <p>Block 0: (0 - binWidth] Block 1: (binWidth - 2 * binWidth] Block 2: (2 * binWidth - 3 *
   * binWidth]
   *
   * <p>Note that covered regions are open on the left ( and closed on the right ].
   *
   * <p>In general, if block i is the ith block (starting from 0), then block i contains all records
   * that have starting position > (i * binWidth) and <= ((i + 1) * binWidth))
   */
  public static class ChrIndex implements htsjdk.tribble.index.ChrIndex {
    ...
    public List<Block> getBlocks(final int start, final int end) {
      if (blocks.isEmpty()) {
        ...
        if (startBinNumber
            >= blocks.size()) // are we off the end of the bin list, so return nothing
        return Collections.emptyList();
        else {
          final int endBinNumber = Math.min((end - 1) / binWidth, blocks.size() - 1);

          // By definition blocks are adjacent for the liner index.  Combine them into one merged
          // block

          final long startPos = blocks.get(startBinNumber).getStartPosition();
          final long endPos =
              blocks.get(endBinNumber).getStartPosition() + blocks.get(endBinNumber).getSize();
    ...
    public void read(final LittleEndianInputStream dis) throws IOException {
      ...
      // largestBlockSize and totalBlockSize are old V3 index values.  largest block size should be
      // 0 for
      // all newer V3 block.  This is a nasty hack that should be removed when we go to V4 (XML!)
      // indices
    ...
    private static ChrIndex mergeBlocks(final ChrIndex idx) {
      final ChrIndex merged =
          new ChrIndex(idx.name, idx.binWidth * 2); // increasing width by 2 each time
      ...
      if (!blocks.hasNext())
        throw new IllegalStateException(
            "Block iterator cannot be empty at the start for " + idx.getName());
