...
/**
 * A wrapper around an {@code InputStream} which performs it's own buffering, and keeps track of the
 * position.
 *
 * <p>TODO: This class implements Positional, which in turn extends LocationAware, which requires
 * preservation of virtual file pointers on BGZF inputs. However, if the inputStream wrapped by this
 * class is a BlockCompressedInputStream, it violates that contract by wrapping the stream and
 * returning positional file offsets instead.
 *
 * @author depristo
 */
public final class PositionalBufferedStream extends InputStream implements Positional {
  ...
  public final int read(final byte[] bytes, final int start, final int len) throws IOException {
    ...
    return 0;
    if (nChars
        < 0) // If no byte is available because the stream is at end of file, the value -1 is
             // returned
    return -1;
    else {
      int nRead = 0;
      ...
      /**
       * Conform to {@link InputStream#read(byte[], int, int)} contract by returning -1 if EOF and
       * no data was read.
       */
  ...
  public static void main(String[] args) throws Exception {
    ...
    for (int i = 0; i < iterations; i++) {
      ...
      final PositionalBufferedStream pbs =
          new PositionalBufferedStream(new FileInputStream(testFile));
  ...
  private static final void printStatus(final String name, long lineCount, double rate, long dt) {
    System.out.printf(
        "%30s: %d lines read.  Rate = %.2e lines per second.  DT = %d%n",
        name, lineCount, rate, dt);
    System.out.flush();
  }
}
