...
public final class VCFHeaderReader {
  ...
  /**
   * Read a VCF header from a stream that may be a VCF file (possibly gzip or block compressed) or a
   * BCF file. After successfully reading a header the stream is positioned immediately after the
   * header, otherwise, if an exception is thrown, the state of the stream is undefined.
   *
  ...
  public static VCFHeader readHeaderFrom(final SeekableStream in) throws IOException {
    ...
    byte[] magicBytes =
        InputStreamUtils.readFully(
            bufferAndDecompressIfNecessary(in), BCFVersion.MAGIC_HEADER_START.length);
    in.seek(initialPos);
    if (magicBytes[0] == '#') { // VCF
  ...
  private static <FEATURE_TYPE extends Feature, SOURCE> VCFHeader readHeaderFrom(
      final InputStream in, final FeatureCodec<FEATURE_TYPE, SOURCE> featureCodec)
      throws IOException {
