...
public final class VCFHeaderReader {
    ...
    public static VCFHeader readHeaderFrom(final SeekableStream in) throws IOException {
        ...
        byte[] magicBytes =
                InputStreamUtils.readFully(bufferAndDecompressIfNecessary(in), BCFVersion.MAGIC_HEADER_START.length);
        in.seek(initialPos);
        if (magicBytes[0] == '#') { // VCF
    ...
    private static <FEATURE_TYPE extends Feature, SOURCE> VCFHeader readHeaderFrom(
            final InputStream in, final FeatureCodec<FEATURE_TYPE, SOURCE> featureCodec) throws IOException {
