...
/**
 * This is an implementation of a Map of {@link JexlVCMatchExp} to true or false values. It lazily
 * initializes each value as requested to save as much processing time as possible.
 */
class JEXLMap implements Map<JexlVCMatchExp, Boolean> {
  /**
   * If a JEXL expression contains values that are not available in the given context, the default
   * behavior is to treat that expression as a miss match.
   */
  public static final JexlMissingValueTreatment DEFAULT_MISSING_VALUE_TREATMENT =
      JexlMissingValueTreatment.TREAT_AS_MISMATCH;
  ...
  /**
   * Construct a new JEXLMap which can evaluate expressions against a specific genotype and variant
   * context
   *
   ...
   */
  public JEXLMap(
      final Collection<JexlVCMatchExp> jexlCollection,
      final VariantContext vc,
      final Genotype g,
      final JexlMissingValueTreatment howToTreatMissingValues) {
   ...
   * @param vc VariantContext to evaluate expressions against
   * @param g genotype to evaluate expressions against, may be null
   *     <p>missing values are treated as false
   */
  public JEXLMap(
      final Collection<JexlVCMatchExp> jexlCollection, final VariantContext vc, final Genotype g) {
   ...
   * @param jexlCollection collection of expressions to be evaluated
   * @param vc VariantContext to evaluate expressions against
   *     <p>missing values are treated as non matches (false)
   */
  ...
  /**
   * Note: due to laziness, this accessor actually modifies the instance by possibly forcing
   * evaluation of an Jexl expression.
   *
   * @throws IllegalArgumentException when {@code key} is {@code null} or when any of the
   *     JexlVCMatchExp (i.e. keys) contains invalid Jexl expressions.
   */
  @Override
  ...
  /**
   * Get all the values of the map, i.e. the {@link Boolean} values. This is an expensive call,
   * since it evaluates all keys that haven't been evaluated yet. This is fine if you truly want all
   * the keys, but if you only want a portion, or know the keys you want, you would be better off
   * using get() to get them by name.
   *
   * <p>Note: due to laziness, this accessor actually modifies the instance by possibly forcing
   * evaluation of an Jexl expression.
   *
   * @return a collection of boolean values, representing the results of all the variants evaluated
   * @throws IllegalArgumentException when any of the JexlVCMatchExp (i.e. keys) contains invalid
   *     Jexl expressions.
   */
  @Override
  ...
  /**
   * Initializes a map and give all keys with null values indicating that they have not yet been
   * evaluated. The actual value will be computed only when the key is requested via {@link
   * #get(Object)} or {@link #values()}.
   *
   * @return an initialized map of jexlExpression -> null
   */
  private static Map<JexlVCMatchExp, Boolean> initializeMap(
      final Collection<JexlVCMatchExp> jexlCollection) {
  ...
  /**
   * Evaluates a {@link JexlVCMatchExp}'s expression, given the current context (and setup the
   * context if it's {@code null}).
   *
   * @param exp the {@link JexlVCMatchExp} to evaluate
   * @return true if the expression matched the context
   * @throws IllegalArgumentException when {@code exp} is {@code null}, or when the Jexl expression
   *     in {@code exp} fails to evaluate the JexlContext constructed with the input VC or genotype.
   */
  private boolean evaluateExpression(final JexlVCMatchExp exp) {
    ...
    try {
      ...
      // this occurs when the jexl expression contained a literal that didn't match anything in the
      // given context
      return howToTreatMissingValues.getMissingValueOrExplode();
    } catch (final JexlException e) {
      // todo - might be better if no exception is caught here but let's user decide how to deal
      // with them; note this will propagate to get() and values()
      throw new IllegalArgumentException(
          String.format("Invalid JEXL expression detected for %s", exp.name), e);
    }
  }

  /**
   * Create a new JexlContext This code is where new JEXL context variables should get added.
   *
