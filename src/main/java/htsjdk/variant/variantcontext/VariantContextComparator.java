...
public class VariantContextComparator implements Comparator<VariantContext>, Serializable {
  ...
  @Override
  public int compare(
      final VariantContext firstVariantContext, final VariantContext secondVariantContext) {
    ...
    final int contigCompare =
        this.contigIndexLookup.get(firstVariantContext.getContig())
            - this.contigIndexLookup.get(secondVariantContext.getContig());
  ...
  /**
   * Returns true if the given header lines are from a file sorted according to this
   * VariantContextComparator. For sorting to work properly, the contig in each header line must
   * have the same index.
   */
  public boolean isCompatible(final Collection<VCFContigHeaderLine> headerLines) {
    for (final VCFContigHeaderLine headerLine : headerLines) {
      ...
      if (existingIndex == null || headerLine.getContigIndex() != existingIndex.intValue())
        return false;
    }
