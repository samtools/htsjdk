...
/**
 * AsyncVariantContextWriter that can be wrapped around an underlying AsyncVariantContextWriter to
 * provide asynchronous output. Records added are placed into a queue, the queue is then drained
 * into the underlying VariantContextWriter by a thread owned by the instance.
 *
 ...
 * @author George Grant
 */
public class AsyncVariantContextWriter extends AbstractAsyncWriter<VariantContext>
  ...
  /**
   * Creates an AsyncVariantContextWriter wrapping the provided VariantContextWriter and using the
   * specified queue size for buffer VariantContexts.
   */
