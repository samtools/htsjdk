...
public class VCFEncoder {
  ...
  private final IntGenotypeFieldAccessors GENOTYPE_FIELD_ACCESSORS =
   ...
   *
   * <p>Depending on the use case it may be more efficient to {@link #write(Appendable,
   * VariantContext)} directly instead of creating an intermediate string.
   *
   * @return the VCF line
   */
  ...
  /**
   * encodes a {@link VariantContext} context as VCF, and writes it directly to an {@link
   * Appendable}
   *
   * <p>This may be more efficient than calling {@link #encode(VariantContext)} and then writing the
   * result since it avoids creating an intermediate string.
   *
  ...
  private void fieldIsMissingFromHeaderError(
      final VariantContext vc, final String id, final String field) {
  ...
  String formatVCFField(final Object val) {
    final String result;
    if (val == null) result = VCFConstants.MISSING_VALUE_v4;
    else if (val instanceof Double) result = formatVCFDouble((Double) val);
      ...
      if (length == 0) return formatVCFField(null);
   ...
   *
   * <p>Large doubles =&gt; gets %.2f style formatting Doubles &lt; 1 / 10 but &gt; 1/100 =&gt; get
   * %.3f style formatting Double &lt; 1/100 =&gt; %.3e formatting
   *
   * @param d
   * @return
   */
  ...
  static boolean isMissingValue(final String s) {
    ...
    return (countOccurrences(VCFConstants.MISSING_VALUE_v4.charAt(0), s) + countOccurrences(',', s)
        == s.length());
  }
        ...
        if (field.equals(VCFConstants.GENOTYPE_KEY)) {
          ...
          if (field.equals(VCFConstants.GENOTYPE_FILTER_KEY)) {
            ...
            if (accessor != null) {
              ...
              if (intValues == null) outputValue = VCFConstants.MISSING_VALUE_v4;
              ...
              if (metaData != null) {
                ...
                if (numInFormatField > 1 && val.equals(VCFConstants.MISSING_VALUE_v4)) {
                  // If we have a missing field but multiple values are expected, we need to
                  // construct a new string with all fields.
   ...
   */
  private void writeInfoString(final Map<String, String> infoFields, final Appendable vcfoutput)
      throws IOException {
  ...
  private static void writeAllele(
      final Allele allele, final Map<Allele, String> alleleMap, final Appendable vcfOutput)
      throws IOException {
