...
/**
 * Writes VariantContext instances to an OutputStream without headers or metadata. For use with
 * SortingCollection ONLY.
 */
public class VCFRecordCodec implements SortingCollection.Codec<VariantContext> {
  ...
  public VCFRecordCodec clone() {
    return new VCFRecordCodec(
        this.vcfEncoder.getVCFHeader(), this.vcfEncoder.getAllowMissingFieldsInHeader());
  }
}
