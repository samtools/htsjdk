...
/**
 * This test is a mechanism to check that none of the data-providers fail to run. It is needed
 * because in the case that a data-provider fails (for some reason, perhaps a change in some other
 * code causes it to throw an exception) the tests that rely on it will be silently skipped. The
 * only mention of this will be in test logs but since we normally avoid reading these logs and rely
 * on the exit code, it will look like all the tests have passed.
 *
...
public class TestDataProviders extends HtsjdkTest {
  ...
  public void independentTestOfDataProviderTest() throws Exception {
    ...
    Assert.assertTrue(
        data.hasNext(), "Found no data from testAllDataProvidersdata. Something is wrong");
  ...
  // runs the BeforeSuite and BeforeClass methods of the same class before it runs the provider
  // itself.
  ...
  public void testDataProviderswithDP(@NoInjection final Method method, final Class clazz)
    ...
    // Some tests assume that the @BeforeSuite methods will be called before the
    // @DataProvidersSet<Method> methodSet = Sets.newHashSet();
