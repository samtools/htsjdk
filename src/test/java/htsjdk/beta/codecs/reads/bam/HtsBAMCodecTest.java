...
public class HtsBAMCodecTest extends HtsjdkTest {
    ...
    public void testBAMDecoder() {
        ...
        try (final BAMDecoder bamDecoder =
                (BAMDecoder) HtsDefaultRegistry.getReadsResolver().getReadsDecoder(inputPath)) {
    ...
    public void testBAMEncoder() {
        ...
        try (final BAMEncoder bamEncoder =
                (BAMEncoder) HtsDefaultRegistry.getReadsResolver().getReadsEncoder(outputPath)) {
            ...
            },
            {
                new Bundle(
                        ...
                        Collections.singletonList(new InputStreamResource(
                                inputPath.getInputStream(), "test bam stream", BundleResourceType.CT_ALIGNED_READS))),
                outputPath
            },
        };
    }
    ...
    private void roundTripFromBundle(final Bundle inputBundle, final IOPath outputPath) {
        try (final BAMDecoder bamDecoder =
                        (BAMDecoder) HtsDefaultRegistry.getReadsResolver().getReadsDecoder(inputBundle);
                final BAMEncoder bamEncoder =
                        (BAMEncoder) HtsDefaultRegistry.getReadsResolver().getReadsEncoder(outputPath)) {
