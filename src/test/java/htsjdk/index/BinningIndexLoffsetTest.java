...
public class BinningIndexLoffsetTest extends HtsjdkTest {
    ...
    @Test
    public void testBinWhoseFirstWindowIsEmptyTakesThePrecedingWindowsOffsetWhenDerivedFromALinearIndex() {
        // Not built for CSI, so the loffset comes from the stored linear index, whose empty leading windows hold 0.
        final BinningIndex.Builder builder = new BinningIndex.Builder(14, 5);
        builder.add(0, 2 * WINDOW + 10, 2 * WINDOW + 10, offset(300), offset(301));
        builder.add(0, 3 * WINDOW, 5 * WINDOW, offset(400), offset(401));
        Assert.assertEquals(loffsetOfBin(builder.build(1).getReference(0), 585), 0);
    }
