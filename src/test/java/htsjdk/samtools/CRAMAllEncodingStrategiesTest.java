...
public class CRAMAllEncodingStrategiesTest extends HtsjdkTest {
    ...
    public Object[][] defaultStrategyRoundTripTestFiles() {
        return new Object[][] {
            ...
            // these tests use lenient equality to only validate read names, bases, alignment start/stop, and qual
            // scores
            ...
            // files created by rewriting the htsjdk test file
            // src/test/resources/htsjdk/samtools/cram/mitoAlignmentStartTest.cram
            ...
            for (final EncodingDescriptor encodingDescriptor : enumerateEncodingDescriptorsFor(dataSeries)) {
                for (final ExternalCompressor compressor : enumerateExternalCompressors(5)) {
                    final String strategyDescription = String.format(
                            ...
                            dataSeries, encodingDescriptor.getEncodingID(), compressor);
    ...
    public void assertRoundTripFidelity(
        ...
        try (final SamReader sourceReader = SamReaderFactory.makeDefault()
                        ...
                        .open(sourceFile);
                final CRAMFileReader copyReader =
                        new CRAMFileReader(targetCRAMFile, new ReferenceSource(referenceFile))) {
    ...
    private Set<EncodingDescriptor> enumerateEncodingDescriptorsFor(final DataSeries ds) {
        ...
        descriptors.add(
                new CompressionHeaderEncodingMap(new CRAMEncodingStrategy()).getEncodingDescriptorForDataSeries(ds));
        if (ds == DataSeries.RN_ReadName) {
            descriptors.add((new ByteArrayLenEncoding(
                            new ExternalIntegerEncoding(ds.getExternalBlockContentId()),
                            new ExternalByteArrayEncoding(ds.getExternalBlockContentId())))
    ...
    public CompressionHeaderEncodingMap createEncodingMapExternalEncodingVariationFor(
            final DataSeries ds, final EncodingDescriptor encodingDescriptor, final ExternalCompressor compressor) {
