...
/** Companion to CRAMBAIIndexerTest, for testing CRAI indices created on cram streams; */
public class CRAMCRAIIndexerTest extends HtsjdkTest {
  ...
  public void testCRAIIndexerFromContainer() throws IOException {
    ...
    CRAMFileReader reader =
        new CRAMFileReader(CRAMFile, null, refSource, ValidationStringency.STRICT);
    ...
    List<CRAIEntry> craiEntries =
        CRAMCRAIIndexer.readIndex(new ByteArrayInputStream(bos.toByteArray())).getCRAIEntries();
  ...
  public void testMultiRefContainer() throws IOException, IllegalAccessException {
    ...
    ReferenceSource source =
        new ReferenceSource(
            new FakeReferenceSequenceFile(samFileHeader.getSequenceDictionary().getSequences()));
    ...
    try {
      CRAMContainerStreamWriter containerWriter =
          new CRAMContainerStreamWriter(cramBAOS, indexBAOS, source, samFileHeader, "test");
    ...
    //        new ByteArrayInputStream(indexBAOS.toByteArray()),
    ...
    Iterator<SAMRecord> it =
        cramReader.query(new QueryInterval[] {new QueryInterval(0, 0, 5)}, false);
  ...
  private static SAMRecord createSAMRecord(
      SAMFileHeader header, int recordIndex, int seqId, int start) {
