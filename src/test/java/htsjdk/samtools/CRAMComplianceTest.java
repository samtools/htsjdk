...
public class CRAMComplianceTest extends HtsjdkTest {
  ...
  public Object[][] getPartialVerificationData() {
    return new Object[][] {
      ...
      {
        "ce#tag_depadded"
      }, // reads with no read bases: https://github.com/samtools/htsjdk/issues/509
      ...
      // {"md#1"},           // fails with "offensive record" errors:
      // https://github.com/samtools/htsjdk/issues/1187
    };
  }
  ...
  // Files that can be subjected to full verification only after read base normalization, because
  // either
  // the reference or the reads contain ambiguity codes that are normalized by
  ...
  private void doComplianceTest(
      final String name, final TriConsumer<Integer, SAMRecord, SAMRecord> assertFunction)
      throws IOException {
    ...
    cramFileReader =
        new CRAMFileReader(
            new FileInputStream(t.cramFile_21),
            (SeekableStream) null,
            source,
            ValidationStringency.SILENT);
    ...
    cramFileReader =
        new CRAMFileReader(
            new FileInputStream(t.cramFile_30),
            (SeekableStream) null,
            source,
            ValidationStringency.SILENT);
    cramFileReaderIterator = cramFileReader.getIterator();
  ...
  private void assertSameRecordsPartial(
      Integer majorVersion, SAMRecord record1, SAMRecord record2) {
    ...
    /**
     * Known issue: CRAM v2.1 doesn't handle reads with missing bases correctly. This causes '*'
     * bases to arise when reading CRAM. Skipping the base comparison asserts.
     * https://github.com/samtools/htsjdk/issues/509
     */
    if (record1.getReadBases() != SAMRecord.NULL_SEQUENCE
        || majorVersion >= CramVersions.CRAM_v3.major) {
  ...
  public void testCRAMThroughBAMRoundTrip(final File originalCRAMFile, final File referenceFile)
      throws IOException {
  ...
  public void testBAMThroughCRAMRoundTrip() throws IOException, NoSuchAlgorithmException {
    ...
    // These files are reduced versions of the CEUTrio.HiSeq.WGS.b37.NA12878.20.21.bam and
    // human_g1k_v37.20.21.fasta
    // files used in GATK4 tests. The first 8000 records from chr20 were extracted; from those
    // around 80 placed but
    // unmapped reads that contained cigar elements were removed, along with one read who's mate was
    // on chr21.
    // Finally all read positions were remapped to the subsetted reference file, which contains only
    // the ~9000 bases
    // used by the reduced read set.
    final File originalBAMInputFile =
        new File(TEST_DATA_DIR, "CEUTrio.HiSeq.WGS.b37.NA12878.20.first.8000.bam");
    ...
    final File tempCRAMFile =
        File.createTempFile("testBAMThroughCRAMRoundTrip", CramIO.CRAM_FILE_EXTENSION);
  ...
  public void testBAMThroughCRAMRoundTripViaPath() throws IOException, NoSuchAlgorithmException {
    ...
    try (FileSystem jimfs = Jimfs.newFileSystem(Configuration.unix())) {
      final Path tempCRAM =
          jimfs.getPath("testBAMThroughCRAMRoundTrip" + CramIO.CRAM_FILE_EXTENSION);
      SAMFileHeader samHeader = getFileHeader(originalBAMInputFile, referenceFile);
  ...
  private SAMFileHeader getFileHeader(final File sourceFile, final File referenceFile)
      throws IOException {
  ...
  private List<SAMRecord> getSAMRecordsFromFile(final File sourceFile, final File referenceFile)
      throws IOException {
    return getSAMRecordsFromPath(sourceFile.toPath(), referenceFile);
  }

  private List<SAMRecord> getSAMRecordsFromPath(final Path sourcePath, final File referenceFile)
      throws IOException {
  ...
  private void writeRecordsToFile(
    ...
    try (final SAMFileWriter writer =
        new SAMFileWriterFactory().makeWriter(samHeader, true, targetFile, referenceFile)) {
