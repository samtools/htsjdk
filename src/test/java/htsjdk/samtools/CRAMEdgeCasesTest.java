...
/**
 * A collection of CRAM test based on round trip comparison of SAMRecord before and after CRAM
 * compression.
 */
public class CRAMEdgeCasesTest extends HtsjdkTest {
  ...
  public void testContigNotFoundInRef() throws IOException {
    ...
    final File CRAMFile =
        new File("src/test/resources/htsjdk/samtools/cram/CRAMException/testContigNotInRef.cram");
    final File refFile =
        new File("src/test/resources/htsjdk/samtools/cram/CRAMException/testContigNotInRef.fa");
  ...
  private void testSingleRecord(SAMRecord record, byte[] ref) throws IOException {
    ...
    CRAMFileWriter cramFileWriter =
        new CRAMFileWriter(baos, source, record.getHeader(), "whatever");
    cramFileWriter.addAlignment(record);
    cramFileWriter.close();
