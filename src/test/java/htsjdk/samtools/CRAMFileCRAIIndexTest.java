...
/**
 * Companion tests for the ones in CRAMFileBAIIndexTest, but run against a .bai that has been
 * converted from a .crai.
 *
 * <p>A collection of tests for CRAM CRAI index write/read that use BAMFileIndexTest/index_test.bam
 * file as the source of the test data. The scan* tests check that for every records in the CRAM
 * file the query returns the same records from the CRAM file.
 */
...
public class CRAMFileCRAIIndexTest extends HtsjdkTest {
  ...
  public void testFileFileConstructor() throws IOException {
    CRAMFileReader reader =
        new CRAMFileReader(tmpCramFile, tmpCraiFile, source, ValidationStringency.STRICT);
  ...
  public void testStreamFileConstructor() throws IOException {
    CRAMFileReader reader =
        new CRAMFileReader(
            new SeekableFileStream(tmpCramFile), tmpCraiFile, source, ValidationStringency.STRICT);
    CloseableIterator<SAMRecord> iterator = reader.queryAlignmentStart("chrM", 1500);
    Assert.assertTrue(iterator.hasNext());
    SAMRecord record = iterator.next();
  ...
  public void testMappedReads() throws IOException {
    ...
    try (SamReader samReader = SamReaderFactory.makeDefault().open(BAM_FILE);
      ...
      Assert.assertEquals(
          samReader.getFileHeader().getSortOrder(), SAMFileHeader.SortOrder.coordinate);
      ...
      while (samRecordIterator.hasNext()) {
        ...
        CloseableIterator<SAMRecord> iterator =
            cramReader.queryAlignmentStart(
        ...
        Assert.assertEquals(
            samRecord.getReferenceName(), cramRecord.getReferenceName(), sam1 + sam2);
  ...
  public void testIteratorConstructor() throws IOException {
    ...
    final CRAMIterator iterator =
        new CRAMIterator(
            new SeekableFileStream(CRAMFile), refSource, boundaries, ValidationStringency.STRICT);
  ...
  public void testNoStringencyIteratorConstructor() throws IOException {
    ...
    long[] boundaries = new long[] {0, (CRAMFile.length() - 1) << 16};
    final CRAMIterator iterator =
        new CRAMIterator(new SeekableFileStream(CRAMFile), refSource, boundaries);
  ...
  public void testIteratorSecondContainerSpan() throws IOException, IllegalAccessException {
    ...
    final Map<Integer, AlignmentSpan> references =
        new ContainerParser(it.getCramHeader().getSamFileHeader())
            .getReferences(secondContainer, ValidationStringency.STRICT);
    it.close();
    ...
    final SAMFileSpan spanOfSecondContainer =
        index.getSpanOverlapping(
            refId, alignmentSpan.getStart(), alignmentSpan.getStart() + alignmentSpan.getSpan());
      ...
      if (record.getReferenceIndex().intValue() == refId) {
        boolean overlaps =
            CoordMath.overlaps(
                ...
                alignmentSpan.getStart(),
                alignmentSpan.getStart() + alignmentSpan.getSpan());
  ...
  public void testQueryInterval() throws IOException {
    ...
    QueryInterval[] query =
        new QueryInterval[] {
          new QueryInterval(0, 1519, 1520), new QueryInterval(1, 470535, 470536)
        };
  ...
  public void testQueryIntervalWithFilePointers() throws IOException {
    ...
    final CloseableIterator<SAMRecord> iterator =
        reader.createIndexIterator(query, false, fileSpan.toCoordinateArray());
    Assert.assertTrue(iterator.hasNext());
    SAMRecord r1 = iterator.next();
    Assert.assertEquals(r1.getReadName(), "3968040");
  ...
  private byte[] cramFromBAM(File bamFile, ReferenceSource source) throws IOException {
    ...
    try (final SamReader reader = SamReaderFactory.makeDefault().open(bamFile);
      ...
      CRAMFileWriter writer =
          new CRAMFileWriter(baos, source, reader.getFileHeader(), bamFile.getName());
      while (iterator.hasNext()) {
        SAMRecord record = iterator.next();
