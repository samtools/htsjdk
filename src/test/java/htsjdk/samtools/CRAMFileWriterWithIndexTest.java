...
public class CRAMFileWriterWithIndexTest extends HtsjdkTest {
  ...
  /**
   * This is to check that the indexing actually works and not just skips records. The approach is
   * to forbid reading of the first container and try accessing reads from the first and the second
   * containers. The first attempt should fail but the second should succeed.
   *
   * @throws IOException
   */
  @Test
  public void testUnnecessaryIO() throws IOException {
    ...
    TabuRegionInputStream tabuIS =
        new TabuRegionInputStream(
            Arrays.asList(new Chunk[] {new Chunk(start, end)}),
            new ByteArraySeekableStream(cramBytes));
    ...
    try {
      // the attempt to read 1st container, which will happen when the iterator is initialized, must
      // throw
  ...
  public void beforeTest() throws Exception {
    ...
    final SAMRecordSetBuilder builder =
        new SAMRecordSetBuilder(false, SAMFileHeader.SortOrder.coordinate);
  ...
  private static void addRandomSequence(
      SAMFileHeader header, int length, InMemoryReferenceSequenceFile rsf) {
