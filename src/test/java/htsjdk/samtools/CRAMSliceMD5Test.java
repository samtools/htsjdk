...
public class CRAMSliceMD5Test extends HtsjdkTest {
  ...
  /**
   * A test case to demonstrate the effect of upper casing of reference bases. The class contains
   * some assertions in the constructor to stress out reference bases case expectations.
   */
  private static class CramTestCase {
    ...
    private CramTestCase() {
      ...
      referenceBasesUpperCased =
          SequenceUtil.upperCase(Arrays.copyOf(referenceBases, referenceBases.length));
      ...
      refBasesFromUCSource =
          referenceSourceUpperCased.getReferenceBases(samFileHeader.getSequence(0), true);
