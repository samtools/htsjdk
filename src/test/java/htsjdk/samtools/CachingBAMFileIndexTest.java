...
public class CachingBAMFileIndexTest extends HtsjdkTest {
  ...
  /**
   * test case to reproduce https://github.com/samtools/htsjdk/issues/1127 this tests for a bug that
   * was hidden on human data pre-hg38 due to a weird interaction between WeakHashMap Integer
   * interning
   */
  @Test
  public void testCachingBamFileIndextest() throws IOException {
    try (final CachingBAMFileIndex index = getIndexWith200Contigs()) {
      ...
      System.gc();
      // contig 1 is never garbage collected because Integer(1) is interned by the jvm and never
      // garbage collectable
      Assert.assertNotNull(index.getQueryResults(1));
  ...
  private static CachingBAMFileIndex getIndexWith200Contigs() throws IOException {
    ...
    final File indexFile =
        new File(outBam.getParent(), IOUtil.basename(outBam) + BAMIndex.BAMIndexSuffix);
