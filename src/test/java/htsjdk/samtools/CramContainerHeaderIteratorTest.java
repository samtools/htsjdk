...
public class CramContainerHeaderIteratorTest extends HtsjdkTest {
    ...
    public void testContainerHeaderIterator() throws IOException {
        ...
        for (int i = 0; i < fullContainers.size(); i++) {
            ...
            Assert.assertEquals(
                    headerOnlyContainer.getContainerHeader().getContainerBlocksByteSize(),
                    fullContainer.getContainerHeader().getContainerBlocksByteSize());
            Assert.assertEquals(headerOnlyContainer.getAlignmentContext(), fullContainer.getAlignmentContext());
            Assert.assertEquals(
                    headerOnlyContainer.getContainerHeader().getNumberOfRecords(),
                    fullContainer.getContainerHeader().getNumberOfRecords());
            Assert.assertEquals(
                    headerOnlyContainer.getContainerHeader().getGlobalRecordCounter(),
                    fullContainer.getContainerHeader().getGlobalRecordCounter());
            Assert.assertEquals(
                    headerOnlyContainer.getContainerHeader().getBaseCount(),
                    fullContainer.getContainerHeader().getBaseCount());
            Assert.assertEquals(
                    headerOnlyContainer.getContainerHeader().getBlockCount(),
                    fullContainer.getContainerHeader().getBlockCount());
            Assert.assertEquals(
                    headerOnlyContainer.getContainerHeader().getLandmarks(),
                    fullContainer.getContainerHeader().getLandmarks());
            Assert.assertEquals(
                    headerOnlyContainer.getContainerHeader().getChecksum(),
                    fullContainer.getContainerHeader().getChecksum());
            ...
            try (SeekableFileStream seekableFileStream = new SeekableFileStream(cramFile)) {
                ...
                Assert.assertEquals(
                        ...
                        fullContainer.getAlignmentContext().getAlignmentStart());
                Assert.assertEquals(
                        container.getAlignmentContext().getAlignmentSpan(),
                        fullContainer.getAlignmentContext().getAlignmentSpan());
                Assert.assertEquals(
                        container.getContainerHeader().getNumberOfRecords(),
                        fullContainer.getContainerHeader().getNumberOfRecords());
                Assert.assertEquals(
                        container.getContainerHeader().getChecksum(),
                        fullContainer.getContainerHeader().getChecksum());
            }
        }
    }
