...
public class DownsamplingIteratorTests extends HtsjdkTest {
  ...
  final EnumMap<Strategy, Double> ACCURACY =
      new EnumMap<Strategy, Double>(Strategy.class) {
        {
  ...
  private void runTests(final String name, final Collection<SAMRecord> recs) {
    for (final DownsamplingIteratorFactory.Strategy strategy :
      ...
      for (final double p : new double[] {0, 0.01, 0.1, 0.5, 0.9, 1}) {
        ...
        final String testcase =
            name + ": strategy=" + strategy.name() + ", p=" + p + ", accuracy=" + accuracy;

        final double readFraction = iterator.getAcceptedFraction();
        Assert.assertEquals(
            out.size(), iterator.getAcceptedCount(), "Mismatched sizes with " + testcase);
        Assert.assertTrue(
