...
public class DuplicateScoringStrategyTest extends HtsjdkTest {
  ...
  public static void testCompare(
      ...
      final DuplicateScoringStrategy.ScoringStrategy strategy,
      final int expected) {
    final SAMRecord rec1 =
        new SAMRecordSetBuilder().addFrag("test", 0, 1, false, false, "36M", null, 2);
    rec1.setFlags(samFlag1);
    final SAMRecord rec2 =
        new SAMRecordSetBuilder().addFrag("test", 0, 1, true, false, "36M", null, 3);
    rec2.setFlags(samFlag2);
    Assert.assertEquals(
        DuplicateScoringStrategy.compare(rec1, rec2, strategy, assumeMateCigar), expected);
  }
}
