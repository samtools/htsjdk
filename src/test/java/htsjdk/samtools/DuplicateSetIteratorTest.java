...
public class DuplicateSetIteratorTest extends HtsjdkTest {
  ...
  public void testSupplementalReads() {
    ...
    DuplicateSetIterator duplicateSetIterator =
        new DuplicateSetIterator(records.iterator(), getSAMRecordSetBuilder().getHeader(), false);
    ...
    // we expect 15 duplicate sets one for the initial two reads and one for each of the additional
    // 14 reads.
    Assert.assertEquals(allSets.size(), 15, "Wrong number of duplicate sets.");
    Assert.assertEquals(
        allSets.get("READ0").size(),
        2,
        "Should be two reads in the READ0 duplicate set, but there are not.");
  }
}
