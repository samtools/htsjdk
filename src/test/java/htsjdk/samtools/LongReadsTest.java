...
public class LongReadsTest extends HtsjdkTest {
    ...
    final IOPath PAC_BIO_READS =
            new HtsPath(TEST_DATA + "NA12878.m64020_190210_035026.chr21.5011316.5411316.unmapped.bam");

    @Test
    public final void testRoundTripBAM() throws IOException {
    ...
    @Test
    public final void testQueryOverlappingForEachRead() throws IOException {
        final IOPath copiedBAMPath = createBAMCopyWithIndex(PAC_BIO_READS);
        ...
        try (final ReadsDecoder sourceDecoder =
                ...
                final ReadsDecoder originalDecoder =
                        HtsDefaultRegistry.getReadsResolver().getReadsDecoder(ReadsBundle.resolveIndex(PAC_BIO_READS));
                final ReadsDecoder copyDecoder = HtsDefaultRegistry.getReadsResolver()
                        .getReadsDecoder(ReadsBundle.resolveIndex(copiedBAMPath))) {
    ...
    @Test
    public final void testQueryContainedForEachRead() throws IOException {
    ...
    @Test
    public final void testQueryUnmapped() throws IOException {
        final IOPath tmpBAMPath = createBAMCopyWithIndex(PAC_BIO_READS);
    ...
    @Test
    public final void testSortOnWrite() throws IOException {
        ...
        try (final ReadsDecoder originalDecoder = HtsDefaultRegistry.getReadsResolver()
                .getReadsDecoder(ReadsBundle.resolveIndex(PAC_BIO_READS), readsDecoderOptions)) {
        ...
        try (final ReadsEncoder bamEncoder = HtsDefaultRegistry.getReadsResolver()
                .getReadsEncoder(new ReadsBundle(sortedOutputPath), new ReadsEncoderOptions().setPreSorted(false))) {
        ...
        try (final ReadsDecoder originalDecoder = HtsDefaultRegistry.getReadsResolver()
                ...
                final ReadsDecoder sortedDecoder =
                        HtsDefaultRegistry.getReadsResolver().getReadsDecoder(sortedOutputPath, readsDecoderOptions);
                final CloseableIterator<SAMRecord> originalIt = originalDecoder.iterator();
            ...
            while (originalIt.hasNext()) {
                ...
                Assert.assertEquals(
                        sortedIt.next().getAlignmentStart(), originalIt.next().getAlignmentStart());
            }
    ...
    private IOPath createBAMCopyWithIndex(final IOPath originalBAM) {
        ...
        // to already exist
        final IOPath tempIndexPath = new HtsPath(tmpBAMPath
                .toPath()
                .resolveSibling(tmpBAMPath.toPath().getFileName().toString().replaceFirst(".bam", ".bai"))
                .toString());

        try (final ReadsDecoder bamDecoder =
                        HtsDefaultRegistry.getReadsResolver().getReadsDecoder(ReadsBundle.resolveIndex(originalBAM));
                final ReadsEncoder bamEncoder = HtsDefaultRegistry.getReadsResolver()
                        .getReadsEncoder(
    ...
    private void compareQueryResults(
            final CloseableIterator<SAMRecord> originalIt, final CloseableIterator<SAMRecord> copyIt) {
        while (originalIt.hasNext()) {
            final SAMRecord originalRecord = originalIt.next();
            Assert.assertTrue(copyIt.hasNext());
            final SAMRecord copyRecord = copyIt.next();
