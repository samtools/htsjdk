...
public class SAMRecordSetBuilderTest extends HtsjdkTest {
    ...
    public static void checkFastaFile(final SAMFileHeader header, final Path fasta, final Path dict)
            throws IOException {
                ...
                if (s.getMd5() != null) {
                    Assert.assertEquals(
                            sequenceFile
                                    ...
                                    .getSequence(referenceSequence.getName())
                                    .getMd5(),
                            s.getMd5());
                }
            });
                ...
                if (md5 != null) {
                    samSequenceDictionary.getSequences().stream().skip(1).forEach(s -> {
                        Assert.assertNotEquals(
                                ...
                                s.getMd5(),
                                "md5s of sequence "
                                        + samSequenceDictionary.getSequence(0).getSequenceName() + " and "
                                        + s.getSequenceName() + " are the same!");
                    });
                }
            }
        }
