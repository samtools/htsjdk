...
public class CRAIEntryTest extends CramRecordTestHelper {
    ...
    public void testGetCRAIEntriesSplittingMultiRef() {
        ...
        assertEntryForSlice(entries.get(0), new ReferenceContext(0), slice1AlnStartOffset, sliceAlnSpan, containerOffset, landmarks[0], sliceByteSize);
        assertEntryForSlice(entries.get(1), new ReferenceContext(1), slice1AlnStartOffset + 1, sliceAlnSpan, containerOffset, landmarks[0], sliceByteSize);
        assertEntryForSlice(entries.get(2), new ReferenceContext(2), slice1AlnStartOffset + 2, sliceAlnSpan, containerOffset, landmarks[0], sliceByteSize);

        // slice 2 has index entries for refs 2, 3, 4

        assertEntryForSlice(entries.get(3), new ReferenceContext(2), slice2AlnStartOffset + 3, sliceAlnSpan, containerOffset, landmarks[1], sliceByteSize);
        assertEntryForSlice(entries.get(4), new ReferenceContext(3), slice2AlnStartOffset + 4, sliceAlnSpan, containerOffset, landmarks[1], sliceByteSize);
        assertEntryForSlice(entries.get(5), new ReferenceContext(4), slice2AlnStartOffset + 5, sliceAlnSpan, containerOffset, landmarks[1], sliceByteSize);
    }
    ...
    private void assertEntryForSlice(final CRAIEntry entry, final Slice slice) {
        assertEntryForSlice(entry, slice.getReferenceContext(),
