...
public class CRAIIndexTest extends HtsjdkTest {
    ...
    private static CRAIEntry updateStart(final CRAIEntry toClone, final int alignmentStart) {
        return new CRAIEntry(toClone.getSequenceId(),
                alignmentStart,
                toClone.getAlignmentSpan(),
                toClone.getContainerStartOffset(),
                toClone.getSliceByteOffset(),
                toClone.getSliceByteSize());
    }

    private static CRAIEntry updateStartContOffset(final CRAIEntry toClone,
    ...
    public void testFind() {
        ...
        CRAIEntry e = new CRAIEntry(sequenceId, 1, 1, 1, 1, 0);
        index.add(e);

        e = updateStartContOffset(e, 2, 2);
        index.add(e);

        e = updateStartContOffset(e, 3, 3);
        index.add(e);
    ...
    public void testGetLeftmost()  {
        ...
        final CRAIEntry e1 = new CRAIEntry(1, 2, 3, 4, 5, 6);
        index.add(e1);
        // trivial case of single entry in index:
        Assert.assertEquals(e1, CRAIIndex.getLeftmost(index));

        final CRAIEntry e2 = updateStart(e1, e1.getAlignmentStart() + 1);
        index.add(e2);
        Assert.assertEquals(e1, CRAIIndex.getLeftmost(index));
    }
    ...
    public void testFindLastAlignedEntry() {
        ...
        for (int lastAligned = 0; lastAligned < indexSize; lastAligned++) {
            ...
            for (int i = 0; i < indexSize; i++) {
                final CRAIEntry e = newEntrySeqStart(i <= lastAligned ? 0 : -1, i);
                index.add(e);
            }
