...
public class CRAIIndexTest extends HtsjdkTest {
  ...
  private boolean allFoundEntriesIntersectQueryInFind(
    ...
    for (final CRAIEntry found : CRAIIndex.find(index, sequenceId, start, span)) {
      ...
      for (int pos = Math.min(found.alignmentStart, start);
          pos <= Math.max(found.alignmentStart + found.alignmentSpan, start + span);
          pos++) {
        if (pos >= found.alignmentStart
            && pos >= start
            && pos <= found.alignmentStart + found.alignmentSpan
            && pos <= start + span) {
          intersects = true;
          break;
        }
  ...
  private void doCRAITest(
      BiFunction<SAMSequenceDictionary, List<CRAIEntry>, SeekableStream> getBaiStreamForIndex)
      throws IOException {
    ...
    final BAMFileSpan span =
        bamIndex.getSpanOverlapping(entry.sequenceId, entry.alignmentStart, entry.alignmentStart);
  ...
  public SeekableStream getBaiStreamFromMemory(
      SAMSequenceDictionary dictionary, final List<CRAIEntry> index) {
    try {
      ...
      final SeekableStream baiStream =
          CRAIIndex.openCraiFileAsBaiStream(
              new ByteArrayInputStream(baos.toByteArray()), dictionary);
  ...
  private SeekableStream getBaiStreamFromFile(
      SAMSequenceDictionary dictionary, final List<CRAIEntry> index) {
    try {
      ...
      final SeekableStream baiStream =
          CRAIIndex.openCraiFileAsBaiStream(
              new SeekableBufferedStream(new SeekableFileStream(file)), dictionary);
      Assert.assertNotNull(baiStream);
      return baiStream;
    } catch (IOException e) {
      throw new RuntimeException(e);
  ...
  public void testFindLastAlignedEntry() {
    ...
    // Scan all allowed combinations of 10 mapped/unmapped entries and assert the found last aligned
    // entry:
