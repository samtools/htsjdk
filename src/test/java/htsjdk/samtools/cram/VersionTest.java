...
public class VersionTest extends HtsjdkTest {
  /**
   * The test purpose is to ensure that a CRAM written by {@link CRAMFileWriter} adheres to CRAM3
   * specs expectations: 1. version 3.+, via both actual byte comparison and CramIO API 2. EOF
   * container 3. trailing 4 bytes of a container bytes are a valid crc32 of previous bytes in the
   * container 3. trailing 4 bytes of a block bytes are a valid crc32 of previous bytes in the block
   *
   * @throws IOException
   */
  @Test
  public void test_V3() throws IOException {
    ...
    byte[] containerHeaderBytes =
        InputStreamUtils.readFully(
            cramSeekableStream, (int) (firstBlockStart - containerStart) - crcByteSize);
