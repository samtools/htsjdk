...
public class CompressionHeaderFactoryTest extends HtsjdkTest {
  ...
  public void testAllEncodingsPresent() {
    final CompressionHeader header =
        new CompressionHeaderFactory()
            .build(new ArrayList<>(), new SubstitutionMatrix(new long[256][256]), true);
    for (final EncodingKey key : EncodingKey.values()) {
      switch (key) {
        ...
        case QQ_scores:
          Assert.assertFalse(
              header.encodingMap.containsKey(key), "Unexpected encoding key found: " + key.name());
          continue;
      }
      Assert.assertTrue(
          header.encodingMap.containsKey(key), "Encoding key not found: " + key.name());
  ...
  public void testAP_delta() {
    ...
    CompressionHeader header =
        new CompressionHeaderFactory()
            .build(new ArrayList<>(), new SubstitutionMatrix(new long[256][256]), sorted);
  ...
  public void test_geByteSizeRangeOfTagValues() {
    ...
    CompressionHeaderFactory.ByteSizeRange range =
        CompressionHeaderFactory.geByteSizeRangeOfTagValues(records, tagID);
  ...
  public void test_getTagType() {
    Assert.assertEquals(
        CompressionHeaderFactory.getTagType(ReadTag.name3BytesToInt("ACi".getBytes())), 'i');
  }
  ...
  public void test_getTagValueByteSize() {
    ...
    Assert.assertEquals(
        CompressionHeaderFactory.getTagValueByteSize((byte) 'Z', "blah-blah"),
        "blah-blah".length() + 1);

    // byte length of an array tag value is: element type (1 byte) + nof bytes (4 bytes) + nof
    // elements * byte size of element
