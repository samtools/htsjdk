...
public class ContainerFactoryTest extends HtsjdkTest {
    ...
    private Iterator<Object[]> getSingleContainerSliceDistribution() {
        final int RECORDS_PER_SLICE = 100;
        return Stream.<Object[]>of(
                        // samRecords, records/slice, slices/container, expected container refContext, expected slice
                        // count,
                        // expected record count/slice, expected slice refContexts,
                        new Object[] {
                            // 1 full single-ref slice with 1 rec
                            QuietTestWrapper.lazy(() -> CRAMStructureTestHelper.createSAMRecordsMapped(1, 0)),
                            RECORDS_PER_SLICE,
                            1,
                        ...
                        new Object[] {
                            ...
                            new ReferenceContext(0),
                            1,
                            Arrays.asList(RECORDS_PER_SLICE),
                            Arrays.asList(new ReferenceContext(0))
                        },
                        new Object[] {
                        ...
                        new Object[] {
                            ...
                            QuietTestWrapper.lazy(() ->
                                    CRAMStructureTestHelper.createSAMRecordsMapped((RECORDS_PER_SLICE * 2) - 1, 0)),
                            RECORDS_PER_SLICE,
                            2,
                            new ReferenceContext(0),
                            2,
                            Arrays.asList(RECORDS_PER_SLICE, RECORDS_PER_SLICE - 1),
                            Arrays.asList(new ReferenceContext(0), new ReferenceContext(0))
                        },
                        new Object[] {
                            ...
                            new ReferenceContext(0),
                            2,
                            Arrays.asList(RECORDS_PER_SLICE, RECORDS_PER_SLICE),
                            Arrays.asList(new ReferenceContext(0), new ReferenceContext(0))
                        },
                        new Object[] {
                        ...
                        new Object[] {
                            ...
                            new ReferenceContext(0),
                            3,
                            Arrays.asList(RECORDS_PER_SLICE, RECORDS_PER_SLICE, RECORDS_PER_SLICE),
                            Arrays.asList(new ReferenceContext(0), new ReferenceContext(0), new ReferenceContext(0))
                        },
                        ...
                        new Object[] {
                            ...
                            QuietTestWrapper.lazy(() -> CRAMStructureTestHelper.createSAMRecordsUnmapped(1)),
                            RECORDS_PER_SLICE,
                            1,
                            ReferenceContext.UNMAPPED_UNPLACED_CONTEXT,
                            1,
                            Arrays.asList(1),
                            Arrays.asList(ReferenceContext.UNMAPPED_UNPLACED_CONTEXT)
                        },
                        new Object[] {
                        ...
                        // NOTE: a mapped reference context can contain unmapped records if they are placed, so really
                        // mapped
                        ...
                        new Object[] {
                            ...
                            QuietTestWrapper.lazy(() -> Stream.of(
                                            Collections.singletonList(
                                                    CRAMStructureTestHelper.createSAMRecordUnmappedPlaced(0, 1)),
                                            CRAMStructureTestHelper.createSAMRecordsMapped(
                                                    (RECORDS_PER_SLICE / 2) - 2, 0),
                                            Collections.singletonList(
                                                    CRAMStructureTestHelper.createSAMRecordUnmappedPlaced(0, 1)))
                                    .flatMap(List::stream)
                            ...
                            new ReferenceContext(0),
                            1,
                            Arrays.asList(RECORDS_PER_SLICE / 2),
                            Arrays.asList(new ReferenceContext(0))
                        },
                        ...
                        new Object[] {
                            // 1 full single-ref slice with RECORDS_PER_SLICE-1 records
                            QuietTestWrapper.lazy(() ->
                                    CRAMStructureTestHelper.createQueryNameSortedSAMRecords(RECORDS_PER_SLICE - 1, 0)),
                            RECORDS_PER_SLICE,
                            1,
                            new ReferenceContext(0),
                            1,
                        ...
                        new Object[] {
                            // use a small number of records from multiple (not coordinate-ordered) reference contexts
                            // to
                            ...
                            QuietTestWrapper.lazy(() -> Stream.of(
                                            ...
                                            CRAMStructureTestHelper.createQueryNameSortedSAMRecords(2, 1))
                                    .flatMap(List::stream)
                                    // even though the individual lists in the stream are sorted, force sort the
                                    // aggregate stream
    ...
    public void testSingleContainerSliceDistribution(
        ...
        for (int i = 0; i < slices.size(); i++) {
            Assert.assertEquals((Integer) slices.get(i).getNumberOfRecords(), expectedSliceRecordCounts.get(i));
            Assert.assertEquals(
                    slices.get(i).getAlignmentContext().getReferenceContext(), expectedSliceReferenceContexts.get(i));
        }
    }

    @DataProvider(name = "multipleContainerSliceDistribution")
    private Iterator<Object[]> getMultipleContainerSliceDistribution() {
        final int RECORDS_PER_SLICE = 100;
        return Stream.<Object[]>of(
                        // input samRecords, records/slice, slices/container,
                        // expected container count, expected record count per container, expected reference context per
                        // container,
                        // expected slices per container
                        new Object[] {
                            // this generates two containers since it has two containers worth of records mapped to a
                            // single ref
                            QuietTestWrapper.lazy(
                                    () -> CRAMStructureTestHelper.createSAMRecordsMapped(RECORDS_PER_SLICE * 2, 0)),
                            RECORDS_PER_SLICE,
                        ...
                        new Object[] {
                            ...
                            2,
                            2,
                            Arrays.asList(RECORDS_PER_SLICE * 2, RECORDS_PER_SLICE + 1),
                            Arrays.asList(new ReferenceContext(0), new ReferenceContext(0)),
                            Arrays.asList(2, 2)
                        },
                        new Object[] {
                            ...
                            2,
                            2,
                            Arrays.asList(RECORDS_PER_SLICE * 2, RECORDS_PER_SLICE * 2),
                            Arrays.asList(new ReferenceContext(0), new ReferenceContext(0)),
                            Arrays.asList(2, 2)
                        },
                        new Object[] {
                            // this generates one container because although it has records mapped to two different
                            // reference
                        ...
                        new Object[] {
                            ...
                            2,
                            2,
                            Arrays.asList(RECORDS_PER_SLICE * 2, RECORDS_PER_SLICE),
                        ...
                        new Object[] {
                            ...
                            RECORDS_PER_SLICE,
                            1,
                            // first container is multi ref and contains half of the records from the first set of
                            // mapped
                            // records; the remaining go into the second container
                            2,
                            Arrays.asList(RECORDS_PER_SLICE, RECORDS_PER_SLICE / 2),
                            Arrays.asList(
                                    ReferenceContext.MULTIPLE_REFERENCE_CONTEXT,
                                    ReferenceContext.UNMAPPED_UNPLACED_CONTEXT),
                            Arrays.asList(1, 1)
                        ...
                        new Object[] {
                            ...
                            QuietTestWrapper.lazy(() -> Stream.of(
                                            CRAMStructureTestHelper.createQueryNameSortedSAMRecords(
                                                    RECORDS_PER_SLICE / 2, 1),
                                            CRAMStructureTestHelper.createQueryNameSortedSAMRecords(
                                                    RECORDS_PER_SLICE / 2, 0),
                                            CRAMStructureTestHelper.createQueryNameSortedSAMRecords(
                                                    RECORDS_PER_SLICE, 1))
                                    .flatMap(List::stream)
                                    // even though the individual lists in the stream are sorted, force sort the
    ...
    public void testMultipleContainerRecordsPerContainer(
        ...
        for (int i = 0; i < containers.size(); i++) {
            ...
            Assert.assertEquals(
                    container.getAlignmentContext().getReferenceContext(), expectedContainerReferenceContexts.get(i));
            Assert.assertEquals(
                    java.util.Optional.of(container.getSlices().size()).get(), expectedSlicesPerContainer.get(i));
        }
    }
}
