...
public class Sam2CramRecordFactoryTest extends HtsjdkTest {
  ...
  /**
   * This checks that all read bases returned in the record from {@link
   * Sam2CramRecordFactory#createCramRecord(SAMRecord)} are from the BAM read base set.
   */
  @Test
  public void testReadBaseNormalization() {
    ...
    final Sam2CramRecordFactory sam2CramRecordFactory =
        new Sam2CramRecordFactory(null, header, CramVersions.CRAM_v3);
    ...
    Assert.assertEquals(
        cramRecord.readBases, SequenceUtil.toBamReadBasesInPlace(record.getReadBases()));
  }
  ...
  public void testAddMismatchReadFeaturesNoReadFeaturesForMatch(
    ...
    final List<ReadFeature> readFeatures =
        buildMatchOrMismatchReadFeatures(refBases, readBases, fastqScores);
    Assert.assertTrue(readFeatures.isEmpty());
  }

  /**
   * Test the outcome of a ACGTN mismatch. The result should always be a {@link Substitution} read
   * feature.
   */
  @Test
  ...
  /**
   * Test the outcome of non-ACGTN ref and read bases mismatching each other. The result should be
   * explicit read base and score capture via {@link ReadBase}.
   */
  @Test
  ...
  private List<ReadFeature> buildMatchOrMismatchReadFeatures(
    ...
    final Sam2CramRecordFactory sam2CramRecordFactory =
        new Sam2CramRecordFactory(refBases.getBytes(), header, CramVersions.CRAM_v3);
    sam2CramRecordFactory.addMismatchReadFeatures(
