...
public class HuffmanTest extends HtsjdkTest {
    ...
    public void testSingleSymbolAlphabetCodeWordLengthIsZero(final Integer testSymbol, final int nWriteReads)
            throws IOException {
        ...
        final HuffmanParams<Integer> huffmanParams =
                new HuffmanParams(Collections.singletonList(testSymbol), Collections.singletonList(0));
    ...
    public void testVerySmallAlphabet(final Map<Byte, Integer> symbolFrequencies, final int maxExpectedCodeLength)
            throws IOException {
    ...
    public Object[][] getCanonicalCodeWords() {
        return new Object[][] {
            ...
            {
                // with narrow symbol frequency variance (5-10), we get little codeword length variance (between 2-3
                // bits)
            ...
            {
                // with wider symbol frequency variance (5-100), we get larger codeword length variance (between 1-4
                // bits)
                new LinkedHashMap<Integer, Tuple<Integer, String>>() {
                    {
    ...
    public void testCanonicalCodeWords(final Map<Integer, Tuple<Integer, String>> symbolCodeWordLengths) {
        ...
        final HuffmanCanoncialCodeGenerator<Integer> codeWordGenerator =
                new HuffmanCanoncialCodeGenerator(huffmanParams);
        ...
        for (final HuffmanBitCode<Integer> hc : codeWords) {
            Assert.assertEquals(hc.getBitCodeWithPrefix(), symbolCodeWordLengths.get(hc.getSymbol()).b);
        }
    }
    ...
    private static <T> HuffmanCanoncialCodeGenerator<T> doHuffmanTest(final Map<T, Integer> symbolFrequencies)
            throws IOException {
        ...
        for (final String sourceCodeString : huffmanCodeStrings) {
            for (final String targetCodeString : huffmanCodeStrings) {
                if (!targetCodeString.equals(sourceCodeString)) {
                    Assert.assertFalse(
                            targetCodeString.startsWith(sourceCodeString),
                            String.format(
                                    "Target huffman code (%s) is prefix for source huffman code (%s)",
                                    targetCodeString, sourceCodeString));
                }
            }
        }
