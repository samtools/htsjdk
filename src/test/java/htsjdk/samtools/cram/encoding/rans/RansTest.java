...
public class RansTest extends HtsjdkTest {
  ...
  public void testRansHeader() {
    byte[] data = randomBytes_GD(1000, 0.01);
    ByteBuffer compressed = RANS.compress(ByteBuffer.wrap(data), RANS.ORDER.ZERO, null);
  ...
  private static void roundTrip(byte[] data) {
    for (RANS.ORDER order : RANS.ORDER.values()) roundTrip(data, order);
  }
  ...
  private static void roundTrip(ByteBuffer data, RANS.ORDER order) {
    ...
    while (data.hasRemaining()) {
      if (!uncompressed.hasRemaining()) Assert.fail("Premature end of uncompressed data.");
  ...
  private void randomBytes_GD(byte[] data, double p) {
    for (int i = 0; i < data.length; i++) data[i] = randomByteGeometricDistribution(p);
  }

  /**
   * A crude implementation of RNG for sampling geometric distribution. The value returned is offset
   * by -1 to include zero. For testing purposes only, no refunds!
   *
