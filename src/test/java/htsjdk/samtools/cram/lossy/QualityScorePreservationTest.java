...
public class QualityScorePreservationTest extends HtsjdkTest {
  ...
  public void test1() {
    ...
    assertEquals(policy0.readCategory.type, ReadCategoryType.LOWER_MAPPING_SCORE);

    assertEquals(policy0.readCategory.param, 999);

    if (policy0.baseCategories != null) assertEquals(policy0.baseCategories.isEmpty(), true);
  ...
  private byte[] compressScores(SAMRecord record, byte[] ref, QualityScorePreservation p) {
    ...
    Sam2CramRecordFactory f =
        new Sam2CramRecordFactory(ref, record.getHeader(), CramVersions.CRAM_v3);
