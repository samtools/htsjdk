...
public abstract class AbstractSRATest extends HtsjdkTest {
  ...
  public final void skipIfCantResolve(Method method, Object[] params) {
    ...
    if (accession != null
        && accession.matches(SRAAccession.REMOTE_ACCESSION_PATTERN)
        && !canResolveNetworkAccession) {
      throw new SkipException(
          "Skipping network SRA Test because cannot resolve remote SRA accession '"
              + checkAccession
              + "'.");
    }
  }
  ...
  /**
   * Exhaust the iterator and check that it produce the expected number of mapped and unmapped
   * reads. Also checks that the hasNext() agrees with the actual results of next() for the given
   * iterator.
   *
   ...
   */
  static void assertCorrectCountsOfMappedAndUnmappedRecords(
      SAMRecordIterator samRecordIterator, int expectedNumMapped, int expectedNumUnmapped) {
