 ...
 *
 * <p>Created by andrii.nikitiuk on 10/28/15.
 */
public class SRAIndexTest extends AbstractSRATest {
  ...
  public void testLevelSize() {
    ...
    Assert.assertEquals(
        index.getLevelSize(0), GenomicIndexUtil.LEVEL_STARTS[1] - GenomicIndexUtil.LEVEL_STARTS[0]);

    Assert.assertEquals(
        index.getLevelSize(LAST_BIN_LEVEL),
        GenomicIndexUtil.MAX_BINS - GenomicIndexUtil.LEVEL_STARTS[LAST_BIN_LEVEL] - 1);
  }
  ...
  public void testBinLocuses(
      SRAAccession acc, int reference, int binIndex, int firstLocus, int lastLocus) {
  ...
  private Object[][] createDataForBinOverlappings() {
    return new Object[][] {
      {DEFAULT_ACCESSION, 0, 1, SRAIndex.SRA_BIN_SIZE, new HashSet<>(Arrays.asList(0))},
      {
        DEFAULT_ACCESSION,
        0,
        SRAIndex.SRA_BIN_SIZE + 1,
        SRAIndex.SRA_BIN_SIZE * 2,
  ...
  public void testBinOverlappings(
      SRAAccession acc, int reference, int firstLocus, int lastLocus, Set<Integer> binNumbers) {
    final SRAIndex index = getIndex(acc);
    final Iterator<Bin> binIterator =
        index.getBinsOverlapping(reference, firstLocus, lastLocus).iterator();
  ...
  private Object[][] createDataForSpanOverlappings() {
    return new Object[][] {
      {DEFAULT_ACCESSION, 0, 1, SRAIndex.SRA_BIN_SIZE, new long[] {0, SRAIndex.SRA_CHUNK_SIZE}},
      {
        ...
        SRAIndex.SRA_BIN_SIZE * 2,
        SRAIndex.SRA_BIN_SIZE * 2 + 1,
        new long[] {0, SRAIndex.SRA_CHUNK_SIZE}
      },
      {
        DEFAULT_ACCESSION,
        0,
        SRAIndex.SRA_CHUNK_SIZE,
        SRAIndex.SRA_CHUNK_SIZE + 1,
        new long[] {
          0, SRAIndex.SRA_CHUNK_SIZE, SRAIndex.SRA_CHUNK_SIZE, SRAIndex.SRA_CHUNK_SIZE * 2
        }
      },
    };
  }
  ...
  public void testSpanOverlappings(
      SRAAccession acc, int reference, int firstLocus, int lastLocus, long[] spanCoordinates) {
    final SRAIndex index = getIndex(acc);
    ...
    Assert.assertTrue(
        Arrays.equals(coordinatesFromIndex, spanCoordinates),
        "Coordinates mismatch. Expected: "
            + Arrays.toString(spanCoordinates)
            + " but was : "
            + Arrays.toString(coordinatesFromIndex));
  }
