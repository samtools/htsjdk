...
public class SRAQueryTest extends AbstractSRATest {
  ...
  private Object[][] createDataForUnmappedCounts() {
    return new Object[][] {{"SRR2096940", 498}};
  }
  ...
  public void testReferenceAlignedCounts(
      String acc, String reference, int referenceStart, int expectedNumMapped) {
    SamReader reader =
        SamReaderFactory.make()
            .validationStringency(ValidationStringency.SILENT)
            .open(SamInputResource.of(new SRAAccession(acc)));
    ...
    final SAMRecordIterator samRecordIterator =
        reader.queryAlignmentStart(reference, referenceStart);
  ...
  public void testQueryCounts(
      ...
      int referenceEnd,
      boolean contained,
      int expectedNumMapped,
      int expectedNumUnmapped) {
    SamReader reader =
        SamReaderFactory.make()
            .validationStringency(ValidationStringency.SILENT)
            .open(SamInputResource.of(new SRAAccession(acc)));
    ...
    final SAMRecordIterator samRecordIterator =
        reader.query(reference, referenceStart, referenceEnd, contained);
