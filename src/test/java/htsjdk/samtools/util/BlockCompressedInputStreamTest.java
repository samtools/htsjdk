...
public class BlockCompressedInputStreamTest extends HtsjdkTest {
  ...
  private static final File BLOCK_UNCOMPRESSED =
      new File("src/test/resources/htsjdk/samtools/util/random.bin");
  private static final File BLOCK_COMPRESSED =
      new File("src/test/resources/htsjdk/samtools/util/random.bin.gz");
  private static final long[] BLOCK_COMPRESSED_OFFSETS =
      new long[] {
  ...
  public void testTruncatedStream() throws Exception {
    ...
    try (BlockCompressedInputStream stream =
        new BlockCompressedInputStream(new ByteArrayInputStream(truncated))) {
    ...
    try (SeekableFileStream sfs = new SeekableFileStream(BLOCK_COMPRESSED)) {
      try (BlockCompressedInputStream stream = new BlockCompressedInputStream(sfs)) {
        ...
        for (int i = 0; i < BLOCK_COMPRESSED_OFFSETS.length - 1; i++) {
          ...
          byte[] expected =
              Arrays.copyOfRange(
                  uncompressed, uncompressed.length - actual.length, uncompressed.length);
  ...
  private static class CountingInflater extends Inflater {
    // Must be static unfortunately, since there's no way to reach down into an inflater instance
    // given a stream
  ...
  private List<String> writeTempBlockCompressedFileForInflaterTest(final File tempFile)
      throws IOException {
    ...
    return new Object[][] {
      // set the default InflaterFactory to a CountingInflaterFactory
      {
        (CheckedExceptionInputStreamSupplier)
            () -> new BlockCompressedInputStream(new FileInputStream(tempFile), false),
        linesWritten,
        4,
        countingInflaterFactory
      },
      {
        (CheckedExceptionInputStreamSupplier) () -> new BlockCompressedInputStream(tempFile),
        linesWritten,
        4,
        countingInflaterFactory
      },
      {
        (CheckedExceptionInputStreamSupplier) () -> new AsyncBlockCompressedInputStream(tempFile),
        linesWritten,
        4,
        countingInflaterFactory
      },
      ...
      // provide a CountingInflaterFactory explicitly
      {
        (CheckedExceptionInputStreamSupplier)
            () ->
                new BlockCompressedInputStream(
                    new FileInputStream(tempFile), false, countingInflaterFactory),
        linesWritten,
        4,
        null
      },
  ...
  public void testCustomInflater(
      ...
      final InflaterFactory customDefaultInflaterFactory)
      throws Exception {
    // clear inflate call counter in CountingInflater
    CountingInflater.inflateCalls = 0;

    // If requested, set the global default InflaterFactory to a custom factory. Otherwise, set it
    // to the default.
    ...
    try (final BufferedReader reader =
        new BufferedReader(new InputStreamReader(bcisSupplier.get()))) {
      String line;
