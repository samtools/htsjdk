...
public class OverlapDetectorTest extends HtsjdkTest {
  ...
  public Object[][] intervalsMultipleContigs() {
    final List<Locatable> input =
        Arrays.asList(new Interval("1", 10, 100), new Interval("2", 200, 300));
    final List<Locatable> empty = new ArrayList<>();
    final List<Locatable> contig1 = Arrays.asList(new Interval("1", 10, 100));
    final List<Locatable> contig2 = Arrays.asList(new Interval("2", 200, 300));

    // returns input, query range, expected SimpleIntervals
    return new Object[][] {
      // we already test elsewhere that it works within a contig, so here we just have to make sure
      // that
  ...
  public void testOverlap(
      final List<Locatable> input, final Locatable query, final Collection<Locatable> expected)
      throws Exception {
  ...
  public Object[][] intervalsSameContig() {
    final List<Locatable> input = Arrays.asList(new Interval("1", 10, 100));
    final List<Locatable> empty = new ArrayList<>();
    final List<Locatable> manyOverlapping =
        Arrays.asList(
    ...
    final List<Locatable> mixExpected =
        Arrays.asList(
            // ends in query interval
            new Interval("1", 10, 60),
            // equal to query interval
  ...
  public void testOverlap(
      final List<Locatable> input, final Interval query, final List<Locatable> expected)
      throws Exception {
  ...
  public void testNoOverlapsAny() throws Exception {
    final List<Locatable> input = Arrays.asList(new Interval("1", 10, 110));
    final OverlapDetector<Locatable> trimmedTargetDetector = new OverlapDetector(20, 20);
    trimmedTargetDetector.addAll(input, input);
    Assert.assertFalse(
        trimmedTargetDetector.overlapsAny(new Interval("1", 50, 85))); // no overlap because of trim
    Assert.assertTrue(
        trimmedTargetDetector
            .getOverlaps(new Interval("1", 50, 85))
            .isEmpty()); // no overlap because of trim
    ...
    Assert.assertTrue(
        untrimmedTargetDetector.overlapsAny(new Interval("1", 50, 85))); // overlaps - no trim
  }
  ...
  public void testAddAllDifferentSizes() throws Exception {
    ...
    final List<Locatable> input1Interval = Arrays.asList(new Interval("1", 11, 101));

    final List<Locatable> input2Intervals =
        Arrays.asList(new Interval("1", 20, 200), new Interval("1", 20, 200));
  ...
  public void testDifferentSizesAddAll() throws Exception {
    ...
    final List<Locatable> l2 =
        Arrays.asList(new Interval("2", 10, 100), new Interval("3", 10, 100));
  ...
  public void testAddTwice() throws Exception {
    final List<Locatable> input =
        Arrays.asList(new Interval("1", 10, 100), new Interval("1", 10, 100));
    final OverlapDetector<Locatable> detector = OverlapDetector.create(input);
