...
public class RelativeIso8601DateTest extends HtsjdkTest {
  ...
  private static final double DELTA_FOR_TIME = 1000;
  ...
  public void testLazyInstance() {
    ...
    Assert.assertEquals(
        lazy.getTime(),
        new Iso8601Date(new Date(System.currentTimeMillis())).getTime(),
        DELTA_FOR_TIME);
    // Assert no exception thrown; this should be valid, because toString should now return an
    // iso-looking date.
  ...
  public void testNonLazyInstance() {
    final long time =
        new Iso8601Date(new Date(System.currentTimeMillis()))
            .getTime(); // ISO strips off milliseconds

    // Test both constructor methods
    final List<RelativeIso8601Date> testDates =
        Arrays.<RelativeIso8601Date>asList(
    ...
    for (final RelativeIso8601Date nonLazy : testDates) {
      ...
      // Assert no exception thrown; this should be valid, because toString return an iso-looking
      // date.
