...
public class SnappyLoaderUnitTest extends HtsjdkTest {
  ...
  public void testCanLoadAndRoundTripWithSnappy() throws IOException {
    ...
    try (Writer out =
        new OutputStreamWriter(snappyLoader.wrapOutputStream(new FileOutputStream(tempFile)))) {
      out.write(toWrite);
    }

    try (LineReader in =
        new BufferedLineReader(snappyLoader.wrapInputStream(new FileInputStream(tempFile)))) {
