...
public class AbstractFeatureReaderTest extends HtsjdkTest {
  ...
  static final String LOCAL_MIRROR_HTTP_INDEXED_VCF_PATH =
      VariantBaseTest.variantTestDataRoot + "ex2.vcf";

  // the "mangled" versions of the files have an extra byte added to the front of the file that
  // makes them invalid
  private static final String TEST_PATH =
      "src/test/resources/htsjdk/tribble/AbstractFeatureReaderTest/";
  ...
  private static final String MANGLED_VCF_TABIX_BLOCK_GZIPPED =
      TEST_PATH + "baseVariants.mangled.vcf.gz";
  private static final String MANGLED_VCF_TABIX_INDEX =
      TEST_PATH + "baseVariants.mangled.vcf.gz.tbi";
  ...
  private static final Function<SeekableByteChannel, SeekableByteChannel> WRAPPER =
      SkippingByteChannel::new;

  /**
   * Asserts readability and correctness of VCF over HTTP. The VCF is indexed and requires and
   * index.
   */
  @Test
  public void testVcfOverHTTP() throws IOException {
    ...
    final AbstractFeatureReader<VariantContext, LineIterator> featureReaderHttp =
        AbstractFeatureReader.getFeatureReader(
            HTTP_INDEXED_VCF_PATH, codec, true); // Require an index to
  ...
  public void testLoadBEDFTP() throws Exception {
    final String path =
        "ftp://ftp.broadinstitute.org/distribution/igv/TEST/cpgIslands with spaces.hg18.bed";
  ...
  @Test(
      dataProvider = "blockCompressedExtensionExtensionStrings",
      dataProviderClass = IOUtilTest.class)
  ...
  public void testBlockCompressionExtensionFile(final String testString, final boolean expected) {
    Assert.assertEquals(
        AbstractFeatureReader.hasBlockCompressedExtension(new File(testString)), expected);
  }

  @Test(
      dataProvider = "blockCompressedExtensionExtensionURIStrings",
      dataProviderClass = IOUtilTest.class)
  ...
  public void testGetFeatureReaderWithPathAndWrappers(
      String file,
      String index,
    ...
    try (FileSystem fs = Jimfs.newFileSystem("test", Configuration.unix());
        final AbstractFeatureReader<VariantContext, ?> featureReader =
            getFeatureReader(file, index, wrapper, indexWrapper, new VCFCodec(), fs)) {
  ...
  @Test(
      dataProvider = "failsWithoutWrappers",
      expectedExceptions = {TribbleException.class, FileTruncatedException.class})
  public void testFailureIfNoWrapper(String file, String index)
      throws IOException, URISyntaxException {
  ...
  private static <T extends Feature> AbstractFeatureReader<T, ?> getFeatureReader(
      String vcf,
      String index,
      Function<SeekableByteChannel, SeekableByteChannel> wrapper,
      Function<SeekableByteChannel, SeekableByteChannel> indexWrapper,
