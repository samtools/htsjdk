...
public class BinaryFeaturesTest extends HtsjdkTest {
  ...
  public Object[][] createData1() {
    return new Object[][] {
      ...
      {new File(TestUtils.DATA_DIR + "bed/Unigene.sample.bed"), new BEDCodec()},
      {
        new File(
            TestUtils.DATA_DIR
                + "bed/NA12878.deletions.10kbp.het.gq99.hand_curated.hg19_fixed.bed"),
        new BEDCodec()
      },
    };
  }
  ...
  public void testBinaryCodec(final File source, final FeatureCodec<Feature, LineIterator> codec)
      throws IOException {
    ...
    final FeatureReader<Feature> binaryReader =
        AbstractFeatureReader.getFeatureReader(
            tmpFile.getAbsolutePath(), new ExampleBinaryCodec(), false);
    ...
    Assert.assertEquals(
        header.get(0), ExampleBinaryCodec.HEADER_LINE, "Failed to read binary header line");
    ...
    while (oit.hasNext()) {
      ...
      Assert.assertTrue(
          bit.hasNext(),
          "Original iterator has items, but there's no items left in binary iterator");
      final Feature bf = bit.next();

      Assert.assertEquals(
          bf.getContig(), of.getContig(), "Chr not equal between original and binary encoding");
      Assert.assertEquals(
          bf.getStart(), of.getStart(), "Start not equal between original and binary encoding");
      Assert.assertEquals(
          bf.getEnd(), of.getEnd(), "End not equal between original and binary encoding");
    }
    Assert.assertTrue(
        !bit.hasNext(),
        "Original iterator is done, but there's still some data in binary iterator");
