...
public class FeatureReaderTest extends HtsjdkTest {
  ...
  public void testLargeNumberOfQueries(
      ...
      final FeatureCodec<Feature, LocationAware> codec)
      throws IOException {
    final AbstractFeatureReader<Feature, LocationAware> reader =
        getReader(featureFile, indexType, codec);
  ...
  public void testBedNames(
    ...
    Assert.assertEquals(
        ...
        "Expected sequences "
            + ParsingUtils.join(",", expectedSequences)
            + " but saw "
  ...
  public void testReadingBeyondIntSizedBlock() throws IOException {
    ...
    for (int chunk = 0; chunk < chunksToRead; chunk++) {
      // System.out.println("Reading chunk " + chunk + " of " + chunkSize + " total read " +
      // totalRead);
