 ...
 *
 * <p>Makes sure that {@link Strand} will behave according to contract in run-time.
 */
public class StrandTest extends HtsjdkTest {
  ...
  public void testEncodeUniqueness() {
    Assert.assertEquals(
        Stream.of(Strand.values()).map(Strand::encode).collect(Collectors.toSet()).size(),
  ...
  public void testEncodeAsCharUniqueness() {
    Assert.assertEquals(
        Stream.of(Strand.values()).map(Strand::encodeAsChar).collect(Collectors.toSet()).size(),
        Strand.values().length);
  }
  ...
  public Object[][] invalidEncodingCharsData() {
    final Character[] testCases =
        new Character[] {(char) 0, 'c', '\t', '\n', (char) -1, 'p', 'n', 'x', '!'};
