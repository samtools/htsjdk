...
public class PassingVariantFilterTest extends HtsjdkTest {
  ...
  public Iterator<Object[]> variantProvider() {
    ...
    final VariantContextBuilder vc_builder =
        new VariantContextBuilder("test", "chr1", 1, 1, Arrays.asList(refA, G));
    ...
    // passing
    variants.add(
        new Object[] {vc_builder.alleles(Arrays.asList(refA, G)).passFilters().make(), true});

    // failing
    variants.add(
        new Object[] {
          vc_builder
              .alleles(Arrays.asList(refA, G))
