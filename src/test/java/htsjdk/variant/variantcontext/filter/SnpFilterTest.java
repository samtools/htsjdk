...
public class SnpFilterTest extends HtsjdkTest {
  ...
  public Iterator<Object[]> variantProvider() {
    ...
    variants.add(
        new Object[] {vc_builder.alleles(Arrays.asList(refA, star)).make(), false}); // SYMBOLIC
    variants.add(
        new Object[] {vc_builder.stop(2).alleles(Arrays.asList(refAG, T)).make(), false}); // INDEL
    variants.add(
        new Object[] {vc_builder.stop(2).alleles(Arrays.asList(refAG, AT)).make(), false}); // MNP
