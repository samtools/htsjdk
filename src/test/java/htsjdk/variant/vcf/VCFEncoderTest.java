...
public class VCFEncoderTest extends HtsjdkTest {
  ...
  public void testVCFWriterDoubleFormatTestData(final double d, final String expected) {
    Assert.assertEquals(
        VCFEncoder.formatVCFDouble(d), expected, "Failed to pretty print double in VCFWriter");
  }
  ...
  public Object[][] makeMissingFormatTestData() {
    ...
    final VariantContextBuilder baseVC =
  ...
  public void testMissingFormatFields(
    ...
    Assert.assertEquals(
        columns[nCol - 1],
        expectedLastColumn,
        "Format fields don't handle missing data in the expected way");
  }
